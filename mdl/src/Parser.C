...
void Parser::help() {
   std::cout << "Usage: mdlparser <options> <switches> mdlFile\n" << std::endl;
   std::cout << "  options (a value is needed):" << std::endl;
   std::cout << "    -i :      List of directories searched for included files. Entries are separated by the : character e.g., ../include:/usr/include:extra/include" << std::endl;
   std::cout << std::endl;
...
int Parser::countOptions(char c) {
	int count = 0;
	for (OptionVector::size_type i = 0; i < getOptions().size(); i++) {
		if (getOption(i).getShortName() != Option::SHORT_NAME_NONE) {
...
Parser::ParameterVector Parser::parse(int argc, char *argv[]) {
	...
	for (StringVector::size_type i = 0, k = 0; i < args.size(); i++, k = i) {
		if (args[i].length() > 2 && args[i].compare(0, 2, "--") == 0) {
			...
			if (option.getType() == Option::TYPE_REQUIRED) {
				if (i + 1 < args.size()) {
			...
			} else if (option.getType() == Option::TYPE_OPTIONAL) {
			...
			int optionals = countType(args[i], Option::TYPE_OPTIONAL);
			int requireds = countType(args[i], Option::TYPE_REQUIRED);
