// =============================================================================
// (C) Copyright IBM Corp. 2005-2025. All rights reserved.
//
// Distributed under the terms of the Apache License
// Version 2.0, January 2004.
// (See accompanying file LICENSE or copy at http://www.apache.org/licenses/.)
//
// =============================================================================

#ifndef MDL_TYPES_H
#define MDL_TYPES_H

// Forward declarations for union types
//class Connection;
#include "Connection.h"
class C_array;
class C_connection;
class C_constant;
class C_dataType;
class C_dataTypeList;
class C_edge;
class C_functor;
class C_general;
class C_generalList;
class C_identifierList;
class C_interface;
class C_interfacePointer;
class C_interfacePointerList;
class C_interfaceMapping;
class C_instanceMapping;
class C_nameComment;
class C_nameCommentList;
class C_node;
class C_noop;
class C_phase;
class C_phaseIdentifier;
class C_phaseIdentifierList;
class C_predicateFunction;
class C_psetMapping;
class C_returnType;
class C_shared;
class C_sharedMapping;
class C_struct;
class C_triggeredFunction;
class C_typeClassifier;
class C_typeCore;
class C_userFunction;
class C_userFunctionCall;
class C_variable;
class Predicate;

#endif