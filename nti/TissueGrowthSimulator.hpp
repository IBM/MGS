...
{
public:
  TissueGrowthSimulator(int size, int rank, Tissue *CTissue, Director* d, SegmentForceDetector* segmentForceDetector,
			SegmentForceAggregator* segForceAggregator, Params *parms, int initialFrontNumber)
    : CurTissue(CTissue), director(d), Detector(segmentForceDetector), Aggregator(segForceAggregator),
      Parms(parms), MaxFrontNumber(0), FrontNumber(0), InitialFrontNumber(initialFrontNumber)
  {
    int localMaxFrontNumber = CTissue->getMaxFrontNumber();
    MPI_Allreduce((void*) &localMaxFrontNumber, (void*) &MaxFrontNumber, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
    if (rank==0) std::cout<<"Max Front Number : "<<MaxFrontNumber<<std::endl;
  }
  ...
  {
    director->iterate(); // director takes you from local state to global state in n steps
    E = Aggregator->aggregate(FrontNumber); // aggregate global forces and energy
  }
