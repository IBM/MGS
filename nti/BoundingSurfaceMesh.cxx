...
BoundingSurfaceMesh::BoundingSurfaceMesh(std::string filename)
  : _hx(0), _hy(0), _hz(0), _npts(0), _ntriangles(0),  _A(0), _B(0), _C(0), _norms(0), _distPtsSqrd(0), _distTrgSqrd(0), 
    _meanX(0), _meanY(0), _meanZ(0), _minDistSqrd(DBL_MAX), boundless(false)
{
  FILE *inputFile;
  ...
  if (myfile.is_open()) {
    ...
    for (int i=0; i<_npts; ++i) {
      ...
      _hy[i]-=_meanY;
      _hz[i]-=_meanZ;
      if ( (_distPtsSqrd[i]=_hx[i]*_hx[i]+_hy[i]+_hy[i]+_hz[i]*_hz[i])<_minDistSqrd)
    ...
    for (int i=0; i<_ntriangles; ++i) {
      ...
      if (_distPtsSqrd[_B[i]]<_distTrgSqrd[i]) _distTrgSqrd[i]=_distPtsSqrd[_B[i]];
      if (_distPtsSqrd[_C[i]]<_distTrgSqrd[i]) _distTrgSqrd[i]=_distPtsSqrd[_C[i]];
    }
    myfile.close();
  }
  else boundless=true;
	...
	if (segDist>_distTrgSqrd[i]) {
	  ...
	  if (nap!=0) {
	    ...
	    if (nap*naq<0) {
	      ...
	      double r[3]={p[0]+tr*(q[0]-p[0]), p[1]+tr*(q[1]-p[1]), p[2]+tr*(q[2]-p[2])};
	      double abr=((r[0]-b[0])*(a[1]-b[1]))-((a[0]-b[0])*(r[1]-b[1]));
	      double bcr=((r[0]-c[0])*(b[1]-c[1]))-((b[0]-c[0])*(r[1]-c[1]));
	      double car=((r[0]-a[0])*(c[1]-a[1]))-((c[0]-a[0])*(r[1]-a[1]));
