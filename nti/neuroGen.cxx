...
int main(int argc, char *argv[])
{
  ...
  for (int branchType=0; branchType<N_BRANCH_TYPES; ++branchType) { // 0:axon, 1:basal, 2:apical
	...
	// only need to do the following once per neuron
	  ...
	  for (int i=0; i<nNeurons; ++i) {
	    ...
	    if ((runningComplexity+=complexities[i])>=targetComplexity) {
	      ...
	      if (neuronBegin==i || runningComplexity-targetComplexity<targetComplexity-(runningComplexity-complexities[i])) {
	...
	if (btissueFile) composite=atof(argv[i+1]);
	else std::cerr<<"Warning : composite file creation supported only for tissue file input!"<<std::endl;
	i++;
      }
	...
	compositeSwcFileNameStream<<compositeSwcFileName<<"."<<rank<<".swc";
	...
	while ( tissueFile.good() ) {
	  ...
	  if (line!="" && line.at(0)!='#')  {
	    if (neuronID>=neuronBegin && neuronID<neuronEnd) {
	      ...
	      std::vector<std::string> results(it, end);
#ifdef DBG
	      for (int j=0; j<results.size(); j++) std::cout << results[j] << std::endl;
#endif
		...
		params[idx] = new NeurogenParams(results.at(PAR_FILE_INDEX+branchType), rank);
		params[idx]->RandSeed=lrandom(rng);
		params[idx]->_rng.reSeedShared(params[idx]->RandSeed);
		...
		std::map<std::string, BoundingSurfaceMesh*>::iterator miter = boundingSurfaceMap.find(params[idx]->boundingSurface);
		if (miter==boundingSurfaceMap.end()) boundingSurfaceMap[params[idx]->boundingSurface] = new BoundingSurfaceMesh(params[idx]->boundingSurface);
	      }
	      ++idx;
	    }
	    neuronID++;
