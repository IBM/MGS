...
InferiorOliveGlomeruliDetector::InferiorOliveGlomeruliDetector(TissueContext* tissueContext)
  :  _tissueContext(tissueContext), _touchVector(0),
...
void InferiorOliveGlomeruliDetector::getTouchPoint(point& p, Touch& t)
{
  ...
  p.x = 0.5*( (c1A[0]+pr1*(c1B[0]-c1A[0]) ) + (c2A[0]+pr2*(c2B[0]-c2A[0]) ) );
  p.y = 0.5*( (c1A[1]+pr1*(c1B[1]-c1A[1]) ) + (c2A[1]+pr2*(c2B[1]-c2A[1]) ) );
  p.z = 0.5*( (c1A[2]+pr1*(c1B[2]-c1A[2]) ) + (c2A[2]+pr2*(c2B[2]-c2A[2]) ) );
}
...
void InferiorOliveGlomeruliDetector::judgeTouches(std::list<int>& sheep, std::list<int>& goats)
{
  std::map<double, std::list<int> >::iterator miter=
    _touchMap.find((*_touchVector)[sheep.back()].getKey1());
  assert(miter!=_touchMap.end());
  std::list<int>::iterator liter=miter->second.begin(), lend=miter->second.end();
...
void InferiorOliveGlomeruliDetector::sparsifyTouches(std::list<int>& sheep, std::list<int>& goats)
{
  ...
  int i,j;
  for (i=0, liter1=sheep.begin(); liter1!=lend && touchStatus[i]==AVAILABLE; ++i) {
    Touch& t1=(*_touchVector)[*liter1];
    for (j=i+1, liter2=++liter1; liter2!=lend && touchStatus[j]==AVAILABLE; ++j, ++liter2) {
...
void InferiorOliveGlomeruliDetector::findGlomeruli(TouchVector* touchVector)
{
  ...
  std::map<double, int>::iterator miter=randomizer.begin(), mend=randomizer.end();
  ...
  double recvbuf[4];
  double sendbuf[4];
  request = MPI::COMM_WORLD.Irecv(recvbuf, 4, MPI::DOUBLE, MPI::ANY_SOURCE, MPI::ANY_TAG);
  ...
  while (!globalDone) {
	...
	while (_touchStatus[ctrIdx]!=AVAILABLE && ++i<nrTouches) ctrIdx=touchOrder[i];
	...
	for (int j=0; j<nrTouches; ++j) {
	  ...
	  if (_touchStatus[candidateIdx]==AVAILABLE ||
		...
		candidates.push_back(candidateIdx);
	      else nonCenter.push_back(candidateIdx);
	    }
	  }
	}
	...
	if (glom.included.size()>=MinTouchesPerGlomerulus) {
	  ...
	  std::list<int>::iterator liter=glom.included.begin(), lend=glom.included.end();
	  assert(ctrIdx==*liter);
	  for (++liter; liter!=lend; ++liter)
	    _touchStatus[*liter]=INCLUDED;
	  lend=candidates.end();
	  ...
	  if (ranks.size()>1) {
	    ...
	    ShallowArray<int>::iterator riter=ranks.begin(), rend=ranks.end();
    ...
    MPI::COMM_WORLD.Allreduce((void*) sentReceived, (void*) totalSentReceived, 2, MPI::INT, MPI::SUM);
    localDone=globalDone=(totalSentReceived[0]==totalSentReceived[1]);
  }
  request.Cancel();
  int totalCount;;
  int nGlomeruli=_glomeruli.size();
  MPI::COMM_WORLD.Allreduce((void*) &nGlomeruli, (void*) &totalCount, 1, MPI::INT, MPI::SUM);
  if (_tissueContext->getRank()==0) printf("Total Glomeruli = %d\n\n", totalCount);
  TouchVector newTouchVector;
  for (int i=0; i<nrTouches; ++i)
    if (_touchStatus[i]==INCLUDED || _touchStatus[i]==CENTER) newTouchVector.push_back(touches[i],0);
  int nrGlomeruliTouches=newTouchVector.getCount();
  totalCount=0;
  MPI::COMM_WORLD.Allreduce((void*) &nrGlomeruliTouches, (void*) &totalCount, 1, MPI::INT, MPI::SUM);
  if (_tissueContext->getRank()==0) printf("Total Glomeruli Touches = %d\n\n", totalCount);
  touches.clear();
  for (int i=0; i<nrGlomeruliTouches; ++i) touches.push_back(newTouchVector[i],0);
}
