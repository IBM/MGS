...
int main(int argc, char* argv[]) {
  double timeStep0, timeStep1, timeStep2, timeStep3, timeStep4, memorySize1,
      memorySize2;
  ;
  int size = 0, rank = 0;
  ...
  bool logTranslations =
      false;                  // SET TO TRUE TO LOG NEURON TRANSLATIONS TO FILE
  ...
  bool dumpResampledNeurons =
      (commandLine.getOutputFormat() == "t") ? true : false;
  NeuronPartitioner* neuronPartitioner =
      new NeuronPartitioner(rank, inputFilename, resample, dumpResampledNeurons,
  ...
  if (ext == "bin") {
    //   if (strcmp(ext, "bin")==0) {
    neuronPartitioner->partitionBinaryNeurons(nSlicers, nTouchDetectors,
                                              tissue);
  } else if (ext == "txt") {
    // else if (strcmp(ext, "txt")==0) {
    neuronPartitioner->partitionTextNeurons(nSlicers, nTouchDetectors, tissue);
  } else {
    std::cerr << "Unrecognized input file extension: " << ext << std::endl;
    exit(0);
  }
  ...
  if (!testParamFile) {
    std::cerr << "Stand alone touch detection requires SynParams.par file!"
              << std::endl;
    exit(0);
  }
  testParamFile.close();
  ...
  ORTouchSpace detectionSpace(electricalSynapseTouchSpace,
                              chemicalSynapseTouchSpace);

  TouchSpace* communicateTouchSpace = new AllInTouchSpace;
  TouchAggregator* touchAggregator = 0;
  // touchAggregator = new TouchAggregator(rank, nTouchDetectors,
  // communicateTouchSpace, 0);  // IF CREATED, TOUCHES ARE AGGREGATED
  ...
  volumeDecomposition =
      new VolumeDecomposition(rank, NULL, nTouchDetectors, tissue, X, Y, Z);
  decomposition = volumeDecomposition;

  TissueSlicer* tissueSlicer = new TouchDetectTissueSlicer(
      rank, nSlicers, nTouchDetectors, tissue, &decomposition, 0, &params,
      MAX_COMPUTE_ORDER);

  TouchDetector* touchDetector = new TouchDetector(
      rank, nSlicers, nTouchDetectors, MAX_COMPUTE_ORDER,
      commandLine.getNumberOfThreads(), commandLine.getAppositionSamplingRate(),
      &decomposition, &detectionSpace, communicateTouchSpace, neuronPartitioner,
      0, &params);
  ...
  TouchAnalyzer* touchAnalyzer = new TouchAnalyzer(
      rank, commandLine.getExperimentName(), nSlicers, nTouchDetectors,
      touchDetector, tissue, maxIterations, touchFilter, true, true);

  std::vector<std::vector<SegmentDescriptor::SegmentKeyData> > touchTableMasks;
  params.getTouchTableMasks(touchTableMasks);
  std::vector<std::vector<SegmentDescriptor::SegmentKeyData> >::iterator
      ttiter = touchTableMasks.begin(),
      ttend = touchTableMasks.end();
  for (; ttiter != ttend; ++ttiter) {
    TouchTable* touchTable = new TouchTable(*ttiter);
    touchTable->setOutput(false);
    touchAnalyzer->addTouchTable(touchTable);  // LIST CREATION :
                                               // PARAMETERIZABLE :
                                               // ANALYSIS-INDEPENDENT TOUCH
                                               // TABLE
  }
  TouchAnalysis* touchAnalysis = new ZeroTouchAnalysis(tissue);
  touchAnalyzer->addTouchAnalysis(
      touchAnalysis);  // LIST CREATION : PARAMETERIZABLE
  ...
  while (!halt) {
    ...
    /*
    if (rank==0) printf("io=%.3f slice=%.3f comm=%.3f tDetection=%.3f
    tTotal=%.3f MemBefore=%.2f MemAfter=%.2f\n",
                        timeStep1 - timeStep0,
                        timeStep2 - timeStep1,
                        timeStep3 - timeStep2,
                        timeStep4 - timeStep3,
