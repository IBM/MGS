...
NeuronPartitioner::NeuronPartitioner(int rank, const std::string& inputFileName,
                                     bool resample, bool dumpOutput,
                                     double pointSpacing)
    : _nSlicers(0),
      _size(0),
      _rank(rank),
      ...
      _pointSpacing(pointSpacing)
{
  ...
  {
    if (_rank == 0)
      std::cerr << "Warning: Geometric spacing factor reset to 1.0!"
                << std::endl;
    _pointSpacing = 1.0;
  }
      ...
      _segmentDescriptor(np._segmentDescriptor)
{
  ...
  // TUAN: why we put 6 here, recommend to replace with a constant in
  // DataConstant.h
  ...
  for (int i = 0; i < _totalNeurons; ++i)
    _neuronSegments[i] = np._neuronSegments[i];
}
...
#ifdef VERBOSE
  if (_rank == 0)
    std::cout << "Read " << _totalNeurons << " neurons..." << std::endl;
#endif
  _neuronSegments = new int[_totalNeurons];
...
void NeuronPartitioner::partitionBinaryNeurons(int& nSlicers,
                                               ...
                                               Tissue* tissue)
{
  ...
  if (_rank < _nSlicers)
    tissue->loadBinary(inputDataFile, _inputFilename, _neuronGroupNeuronCount,
                       _neuronGroupSegCount, _neuronGroupStartNeuron, this,
                       _resample, _dumpOutput, _pointSpacing);
  tissue->getTotalNumberOfBranches();

  fclose(inputDataFile);
}

void NeuronPartitioner::countAllNeurons(const std::string& inputFilename,
                                        int& totalNeurons,
                                        int& totalSegmentsRead,
                                        int* neuronsPerLayer,
                                        std::vector<int>& neuronSegments)
{
  ...
  Params p;
  char bufS[1024], filename[256], offsetType, axonPar[256], basalPar[256],
      apicalPar[256];
...
void NeuronPartitioner::partitionTextNeurons(int& nSlicers,
                                             const int nTouchDetectors,
                                             Tissue* tissue)
{
  _size = (nSlicers > nTouchDetectors) ? nSlicers : nTouchDetectors;
  ...
  if (_rank == 0)
    countAllNeurons(_inputFilename, _totalNeurons, _totalSegmentsRead,
                    _neuronsPerLayer, neuronSegments);
  ...
  if (_rank == 0)
    for (int i = 0; i < _totalNeurons; ++i)
      _neuronSegments[i] = neuronSegments[i];
  ...
  tissue->setPartitioner(this);
  if (_rank < _nSlicers)
    tissue->loadText(_inputFilename, _neuronGroupNeuronCount,
                     _neuronGroupSegCount, _neuronGroupStartNeuron, this,
                     _resample, _dumpOutput, _pointSpacing);
  tissue->getTotalNumberOfBranches();
}
...
void NeuronPartitioner::decompose()
{
  ...
  for (int i = 0; i < _nSlicers; ++i)
  {
    ...
    segsPerProc =
        double(_totalSegmentsRead - segsAlloc) / double(_nSlicers - i);
    ...
    while (_totalNeurons - currentNeuron > _nSlicers - i - 1)
    {
      ...
      if (currentNeuron >= _totalNeurons ||
          segsPerProc - double(segsAllocHere) <
              0.5 * double(_neuronSegments[currentNeuron]))
      {
        break;
      }
    }
...
void NeuronPartitioner::getRanks(Sphere* sphere, double* coords2,
                                 double deltaRadius,
                                 ShallowArray<int, MAXRETURNRANKS, 100>& ranks)
{
  ranks.clear();
  ranks.push_back(
      getNeuronRank(_segmentDescriptor.getNeuronIndex(sphere->_key)));
}

void NeuronPartitioner::addRanks(Sphere* sphere, double* coords2,
                                 double deltaRadius,
                                 ShallowArray<int, MAXRETURNRANKS, 100>& ranks)
{
  ranks.push_back(
...
bool NeuronPartitioner::mapsToRank(Sphere* sphere, double* coords2,
                                   double deltaRadius, int rank)
{
  return (rank ==
          getNeuronRank(_segmentDescriptor.getNeuronIndex(sphere->_key)));
}

void NeuronPartitioner::getRanks(Sphere* sphere, double deltaRadius,
                                 ShallowArray<int, MAXRETURNRANKS, 100>& ranks)
{
  ranks.clear();
  ranks.push_back(
