...
strcpy(inputFilename, commandLine.getInputFileName().c_str());*/
  ...
  bool dumpResampledNeurons = ((commandLine.getOutputFormat() == "t" ||
                                commandLine.getOutputFormat() == "bt"))
                                  ? true
                                  : false;
  ...
  if (nSegmentForceDetectors == 0 || nSegmentForceDetectors > size)
    nSegmentForceDetectors = size;
  ...
  if (ext == "bin") {
    // if (strcmp(ext, "bin") == 0) {
    neuronPartitioner->partitionBinaryNeurons(nSlicers, nSegmentForceDetectors,
                                              tissue);
  } else {
    neuronPartitioner->partitionTextNeurons(nSlicers, nSegmentForceDetectors,
                                            tissue);
  }
  volumeDecomposition = new VolumeDecomposition(
  ...
  SegmentForceAggregator* segmentForceAggregator = new SegmentForceAggregator(
      rank, nSlicers, nSegmentForceDetectors, tissue);
...
#ifdef VERBOSE
  if (rank == 0)
    std::cerr << "Max Branch Order = " << tissue->getMaxBranchOrder()
              << std::endl;
#endif
  ...
  SegmentForceDetector* segmentForceDetector = new SegmentForceDetector(
      rank, nSlicers, nSegmentForceDetectors, commandLine.getNumberOfThreads(),
  ...
  TissueGrowthSimulator TissueSim(size, rank, tissue, director,
                                  segmentForceDetector, segmentForceAggregator,
                                  &params, commandLine.getInitialFront() - 1);
  ...
  NeuroDevTissueSlicer* neuroDevTissueSlicer = new NeuroDevTissueSlicer(
      rank, nSlicers, nSegmentForceDetectors, tissue, &decomposition,
      &frontSegmentSpace, &params, segmentForceDetector->getEnergy());

#ifdef VERBOSE
  if (rank == 0)
    std::cerr << "Maximum Front level = " << TissueSim.getMaxFrontNumber()
              << std::endl;
#endif
  ...
  while (grow) {
    ...
    do {
#ifdef USING_CVC
      if (clientConnect) {
        tissue->getVisualizationSpheres(frontLimitedSegmentSpace, nspheres,
      ...
      En = E;
      now = MPI_Wtime();
      if (rank == 0 && iteration < maxIterations)
  ...
  if (commandLine.getOutputFormat() == "t" ||
      commandLine.getOutputFormat() == "bt") {
    ...
    if (commandLine.getOutputFileName() != "") {
      ...
      while (nextToWrite < size) {
        ...
        if (nextToWrite == rank) {
          ...
          segmentsWritten = tissue->outputTextNeurons(
