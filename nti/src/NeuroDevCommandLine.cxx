...
bool NeuroDevCommandLine::parse(std::vector<std::string>& argv)
{
	...
	for (NeuroDevParser::ParameterVector::size_type i = 0; i < parameterVector.size(); i++) {
		...
		if (_verbose)
		    std::cout << "Initial front set : " << value << ".\n";
		...
		if (_verbose)
		    std::cout << "Input filename set : " << value << ".\n";
		_inputFileName = value;
	    } else if (option.getShortName() == 'j') {
		if (_verbose)
		    std::cout << "Number of threads set : " << value << ".\n";
		_nThreads = atoi(value.c_str());
	    } else if (option.getShortName() == 'k') {
		if (_verbose)
		    std::cout << "Cell body migration set.\n";
		...
		if (_verbose)
		    std::cout << "Decomposition set : " << value << ".\n";
		...
		if (_verbose)
		    std::cout << "Number of slicers set : " << value << ".\n";
		_nSlicers = atoi(value.c_str());
	    } else if (option.getShortName() == 't') {
		if (_verbose)
		    std::cout << "Time step set.\n";
...
bool NeuroDevCommandLine::parse(const char* line)
{
  ...
  */
	...
	std::cerr << "NeuroDevCommandLine: Too many arguments! (limits to : " << ConstantData::MaxNumCmdLineArgs << ")" << std::endl;
	exit(0);
    }
