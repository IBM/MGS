...
#ifndef LTWT_TOUCH
           <<_touchData[4]<<" "
           <<_endTouch[0]<<_endTouch[1]<<_endTouch[2]<<_endTouch[3]<<" "
#endif
	  ;
}
...
{
  double rval=0;
  if (key==getKey1()) rval=getKey2();
  else if (key==getKey2()) rval=getKey1();
  return rval;
}
...
{
  double rval=DBL_MAX;
  if (key==getKey1()) rval=getProp1();
  else if (key==getKey2()) rval=getProp2();
  return rval;
}
