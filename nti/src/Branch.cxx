...
{
  // std::memcpy(_displacedTerminalCoords, b._displacedTerminalCoords,
  // 3*sizeof(double));
  std::copy(b._displacedTerminalCoords, b._displacedTerminalCoords + 3,
            _displacedTerminalCoords);
}
...
double Branch::getLength() const
{
  ...
  // _segments[_numberOfSegments-1].getCoords()));
  return _segments[_numberOfSegments - 1].getDist2Soma() -
         _segments[0].getDist2Soma();
}
  ...
  else
  {
    ...
    {// seg2 is the Segment that is far enough from seg1
      while (1)
      {
        while (1)
        {
          double d = 2.0 * L * seg1->getRadius() /
                     ((L / pointSpacing) + seg1->getRadius() -
                      seg2->getRadius());  // see P1010005.jpg
    ...
    {
      double d =
          2.0 * L * seg1->getRadius() /
          (L + seg1->getRadius() - seg2->getRadius());  // see P1010005.jpg
      if (fabs(d) <= L)
      {
        newSeg = _segments[0];
        newSeg.setSegmentIndex(segCount);
    ...
    else
    {
      ...
      _resampledTerminalIndex =
          _rootSegment->getBranch()->getResampledTerminalIndex();
      segments.resize(--s);
    }
  }
...
void Branch::resetBranchRoots(std::vector<Segment>& segments)
{
  ...
  else
  {
    ...
    bool newRoot = false;
    for (int k = 0; k < 3; ++k)
      newRoot = newRoot || (rootSegCoords[k] != firstSegCoords[k]);
    if (newRoot)
    {
      segments.resize(s + 1);
    ...
    {
      segments[s - segCount].getRadius() =
          segments[s - segCount + 1].getRadius();
    }
  }
  _numberOfSegments = segCount;
}

Segment* Branch::loadBinary(FILE* inputDataFile, Segment* segmentPtr,
                            Neuron* neuron, const int branchIndex)
{
  ...
  for (int i = 0; i < _numberOfSegments; ++i, ++segmentPtr)
  {
    ...
    if (segmentPtr->getRadius() == 0) segmentPtr->getRadius() = DEFAULT_RADIUS;
  }
  setDisplacedTerminalCoords();
  return segmentPtr;
}

Segment* Branch::loadText(FILE* inputDataFile, Segment* segmentPtr,
                          Neuron* neuron, const int branchIndex,
                          std::list<int>& branchTerminals, double xOffset,
                          ...
                          int cellBodyCorrection)
{
  ...
  {  // correction added when NeuroMorpho.org allowed multiple cell bodies in a
     // single swc file
  ...
  while (1)
  {
    ...
    {
      std::cerr << "Branch.cxx : Only one cell body allowed per file!"
                << std::endl;
      exit(0);
    }
    assert(parent < seg);
    if (r == 0) r = DEFAULT_RADIUS;
    segmentPtr->loadText(x + xOffset, y + yOffset, z + zOffset, r, this,
                         _numberOfSegments);
    prevSeg = seg;
    ++_numberOfSegments;
    ++segmentPtr;
...
void Branch::findRootSegment()
{
  ...
  for (int k = 0; k < _branchIndex; ++k)
  {
    ...
    {
      Segment& lastseg =
          branches[k].getSegments()[branches[k].getNumberOfSegments() - 1];
...
// INPUT:
//    dist2Soma = the along-branch-distance from the first Segment of that
//    Branch to the soma
void Branch::setDist2Soma(double dist2Soma)
{
  ...
  {
    dist2Soma +=
        sqrt(SqDist(_segments[i - 1].getCoords(), _segments[i].getCoords()));
...
void Branch::resetCoordinates(Segment& newSeg, Segment* seg2, Segment* seg3,
                              double d, double L, double pointSpacing)
{
  assert(d > 0);
  double ratio = d / L;
...
// GOAL: find the index 'i' of the Segment (represented as seg2)
//  so that it has the distance to seg1 is large enough
//  large enough ~ the point of one Segment does not inside the other Segment's sphere
// RETURN: The index 'i' and the satisfied distance 'L'
