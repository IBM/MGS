...
LENSTissueSlicer::LENSTissueSlicer(
  ...
  TissueSlicer(rank,
	       nSlicers,
	       nTouchDetectors,
	       tissueContext->_tissue,
	       &tissueContext->_decomposition,
	       params),
  ...
  _sliced(false)
{
  ...
  MPI_Type_create_hindexed(1, &blocklen, &disp, MPI_DOUBLE, &typeSegmentDataBasic);
  MPI_Type_create_resized(typeSegmentDataBasic, 0, 
			  sizeof(Capsule), &_typeSegmentData);
...
void LENSTissueSlicer::sliceAllNeurons()
{
  if (_tissueContext->_touchVector.getBlockCount()>0 && !_sliced) {
    ...
    for (TouchVector::TouchIterator titer=_tissueContext->_touchVector.begin(); titer!=tend; ++titer) {
      ...
      int v1Idx=-1;
      if (_segmentDescriptor.getFlag(key1) && _tissueContext->isTouchToEnd(c1,*titer)) {
	Sphere sphere;
	...
	v1Idx=decomposition->getRank(c1.getSphere());
      ...
      int v2Idx=-1;
      if (_segmentDescriptor.getFlag(titer->getKey2()) && _tissueContext->isTouchToEnd(c2,*titer)) {
	Sphere sphere;
	...
	/*
	else if (v1Idx!=_rank) // This else only required for neuron decomposition
	  _sliceSegmentIndices[v2Idx].push_back(c1Idx);
	*/
      }
    }
    for (int i=0; i<_numberOfReceivers; ++i) {
      ...
      std::vector<long int>::iterator newEnd = unique(_sliceSegmentIndices[i].begin(), _sliceSegmentIndices[i].end());
...
void LENSTissueSlicer::writeBuff(int i, int j, int& writePos)
{
  ...
  memcpy(&_sendBuff[writePos], _tissueContext->_capsules[(_sliceSegmentIndices[i])[j]].getData(), _dataSize*sizeof(double));
  writePos += _dataSize;
}
