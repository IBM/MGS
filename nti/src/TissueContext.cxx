...
//
//    int rank1, rank2;
//    bool result1 = isPartOfExplicitJunction(capsule,t, status, rank1) ;
//    bool result2 =
//        isPartOfImplicitJunction(capsule,t, status, rank2);
//    bool result = result1 or result2;
//    if (result)
//    {
//        if (result1)
//            rank = rank1;
...
//bool TissueContext::isPartOfJunction(Capsule& capsule, Touch &t, int & rank, Capsule* junctionCapsule)
//{
//    int status;
//    bool result = false;
//    ComputeBranch* branch = capsule.getBranch();
...
//        }
//    }
//    if (isPartOfImplicitJunction(capsule, t, status, rank))
//    {
//        result = true;
//        switch (status) 
//        {
...
//bool TissueContext::isPartOfJunction(Capsule& capsule, Touch &t)
//{
//
//    int status;
//    int rank;
//    return (isPartOfExplicitJunction(capsule,t, status, rank) or
//            isPartOfImplicitJunction(capsule,t, status, rank));
//}

//   status ==  1 (proximal) the capsule belong to
//              2 (distal)
//              -1 (undefined)
//bool TissueContext::isPartOfImplicitJunction(Capsule& capsule, Touch &t, 
//        int & status, int & rank)
//{
//    //NOTE: check the original implementation 'TouchDetectTissueSlicer::sliceAllNeurons()'
//    //which calll Segment->isJunctionSegment(true) to configure the flag
...
//      if (reserved4proxend > 0.0 and
//              parentBranch->_daughters.size() > 1
//              /*ensure not slicing cut middle of tree*/)
//      {
//          if (cps_index < reserved4proxend)
//              result = true;
//          if (cps_index == int(floor(reserved4proxend)))
...
//          if (result)
//          {
//              ComputeBranch* parentBranch = capsule.getBranch()->_parent;
//              status = CapsuleAtBranchStatus::PROXIMAL;
//              rank = _decomposition->getRank(parentBranch->lastCapsule()->getSphere());
//          }
//      }
//      if (reserved4distend > 0.0 and
//              branch->_daughters.size() > 1
//              /*ensure not terminal or slicing cut*/)
//      {
//          if (cps_index_reverse < reserved4distend)
//              result = true;
//          if (cps_index_reverse == int(floor(reserved4distend)))
...
//          {
//              status = CapsuleAtBranchStatus::DISTAL;
//              rank = _decomposition->getRank(capsule.getSphere());
//          }
//      }
//  }
//  if (_segmentDescriptor.getBranchType(capsule.getKey()) ==
...
//GOAL: put onto the MPI process that hold the first point of the capsule
//int TissueContext::getImplicitJunctionMPIRank(Capsule& caps)
//{
//    int rank;
//    unsigned int computeOrder =
//        _segmentDescriptor.getComputeOrder(caps.getKey());
//    //if (_segmentDescriptor.getBranchType(caps.getKey()) ==
//    //        Branch::_SOMA)  // the branch is a soma
//    //    rank = this->getRankOfEndPoint(caps.getBranch());
//    //else
//    {
//        ComputeBranch* branch = caps.getBranch();
//        assert(branch->_configuredCompartment);
...
//        float reserved4proxend = branch->_numCapsulesEachSideForBranchPoint.first;
//        int cps_index =
//            (&caps - caps.getBranch()->_capss);  // zero-based index
//
//        //# capsules in that branch
//        int ncaps = branch->_nCapsules;
//        int cps_index_reverse = ncaps - cps_index - 1;  // from the distal-end
//        if (computeOrder == 0)
//            rank = this->getRankOfEndPoint(caps.getBranch()->_parent);
...
//int TissueContext::getExplicitJunctionMPIRank(Capsule& caps)
//{
//    int rank;
//    unsigned int computeOrder =
//        _segmentDescriptor.getComputeOrder(caps.getKey());
