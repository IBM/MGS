...
void NeuroDevParser::help() {
  ...
  if (rank==0) {
   ...
   std::cout << "    -a : apposition-sampling-rate : Prior to synapse creation sampling, touch sampling rate."<< std::endl;
   std::cout << "    -b : binary-input-file : Binary structural input file name."<<std::endl;
   std::cout << "    -c : client-connect : Connect to visualization environment." << std::endl;
   std::cout << "    -d : n-detectors : Number of processors used for touch and force detection. Cardinal value 0 yields MPI_Comm_size. Must be power of 3 or product of slicing geometry."<< std::endl;
   std::cout << "    -e : energy-conv-crit : Energy convergence criterion. MD concept used in force field iteration."<< std::endl;
   std::cout << "    -f : initial-front : Initial front from which growth begins. Must be >= 0."<< std::endl; 
   std::cout << "    -g : geometric-resampling-factor : Tangent sphere spacing, multiple of [r(n)+r(n+1)]."<< std::endl;
   std::cout << "    -h : help" << std::endl; 
   std::cout << "    -i : text-input-file : Input tissue specification file name."<< std::endl;
   std::cout << "    -j : threads : Number of threads used for touch and force detection inner loop."<< std::endl;
   std::cout << "    -k : cell-body-migration : Initial position of cell bodies determined by tissue interactions."<< std::endl;
   std::cout << "    -m : max-iterations : Maximum number of iterations. Must be >= 1."<< std::endl;
   std::cout << "    -n : decomposition : Computational decomposition : volume (default), neuron (experimental)."<< std::endl;
   std::cout << "    -o : output-file : Output file name (text or binary)."<< std::endl;
   std::cout << "    -p : param-file : Parameter file name."<< std::endl;
   std::cout << "    -r : compartment-resampling-factor : Number of capsules per physiological compartment, resampled following touch detection."<< std::endl;
   std::cout << "    -s : n-slicers : Number of processors used for column slicing. Cardinal value 0 yields MPI_Comm_size. Must be <= number of neurons."<< std::endl; 
   std::cout << "    -t : time-step : Time step. MD concept usesd in force field iteration."<< std::endl;
   std::cout << "    -u : dump-all-output : output file format : text 't' (default) | binary 't' | text and binary 'bt'."<< std::endl;
   std::cout << "    -x : slicing-geom-x : X dimension of slicing geometry."<< std::endl;
   std::cout << "    -y : slicing-geom-y : Y dimension of slicing geometry."<< std::endl;
   std::cout << "    -z : slicing-geom-z : Z dimension of slicing geometry."<< std::endl;
...
int NeuroDevParser::countOptions(String arg) {
	...
	for (OptionVector::size_type i = 0; i < getOptions().size(); i++) {
		if (getOption(i).getLongName() != NeuroDevParser::Option::ND_LONG_NAME_NONE) {
...
int NeuroDevParser::countOptions(char c) {
	int count = 0;
	for (OptionVector::size_type i = 0; i < getOptions().size(); i++) {
		if (getOption(i).getShortName() != NeuroDevParser::Option::ND_SHORT_NAME_NONE) {
...
int NeuroDevParser::countArgs(StringVector &args, StringVector::size_type start) {
	int count = 0;
...
NeuroDevParser::Option NeuroDevParser::Option::ND_OPTION_NONE = NeuroDevParser::Option(NeuroDevParser::Option::ND_SHORT_NAME_NONE, NeuroDevParser::Option::ND_LONG_NAME_NONE, NeuroDevParser::Option::TYPE_NONE);

NeuroDevParser::Option::Option(char shortName, std::string longName, Type type) {
...
NeuroDevParser::Option &NeuroDevParser::Option::operator=(const NeuroDevParser::Option &option) {
