...
NeuroDevTissueSlicer::NeuroDevTissueSlicer(const int rank, const int nSlicers,
  ...
  for (int i = 0; i < _segsSize; ++i) {
    if (_segs[i].getSegmentIndex() == 0)
      _R0a[i] = 0;
    else
      _R0a[i] =
          sqrt(SqDist(_segs[i].getOrigCoords(), _segs[i - 1].getOrigCoords())) *
          _params->getBondR0(
              _segmentDescriptor.getBranchType(_segs[i].getSegmentKey()));
    _R0b[i] = -1.0;
  }
}
...
void NeuroDevTissueSlicer::sliceAllNeurons() {
  if (!_tissue->isEmpty()) {
    ...
    for (int idx = 0; seg < _segsEnd; ++seg, ++idx) {
      // eliminate first segment of branches, as these are redundant for the
      // purposes of neuroDev
      if (_segmentSpace->isInSpace(seg) && seg->getSegmentIndex() > 0) {
        computeTopologicalForces(idx);
        decomposition->getRanks(
            &seg->getSphere(),
            _params ? _params->getRadius(seg->getSegmentKey()) : 0, indices);
        end = indices.end();
...
void NeuroDevTissueSlicer::computeTopologicalForces(int idx) {
  ...
  if (lengthK0 != 0.0 || distortionK0 != 0.0) {
    ...
    if (lengthK0 != 0.0) {
      _segmentForce.HarmonicDistance(prevCds, cds, _R0a[idx], lengthK0, E, F);
    ...
    if (distortionK0 != 0.0) {
      double* cdsCB =
          seg->getBranch()->getNeuron()->getSegmentsBegin()->getCoords();
      ...
      _segmentForce.HarmonicDistance(cdsT2, cds, _R0b[idx], distortionK0, E, F);
      _E += E;
      for (int ii = 0; ii < 3; ++ii) Fa[ii] += F[ii];
    }
  }
...
void NeuroDevTissueSlicer::writeBuff(int i, int j, int& writePos) {
  ...
  // sizeof(double));
  writePos += WRITE_SZ1;
  std::copy(&_forces[3 * (_sliceSegmentIndices[i])[j]],
            &_forces[3 * (_sliceSegmentIndices[i])[j]] + WRITE_SZ2,
            &_sendBuff[writePos]);
  // memcpy(&_sendBuff[writePos], &_forces[3 * (_sliceSegmentIndices[i])[j]],
