    ...
    _experimentName("")
{
  ...
  *_capsules = new Capsule[_previousPhase1RecvBufSize];
  ...
  if (_communicateTouchSpace || _tissueContext) { 
    ...
    for (int i=0; i<_numberOfReceivers; ++i) {
      int numTouches = 1;
      MPI_Type_create_hindexed(numTouches, _touchBlockLengths, _touchBlockDisplacements, *(Touch::getTypeTouch()), &_typeTouches[i]);
...
void TouchDetector::prepareToReceive(int receiveCycle, int receivePhase, CommunicatorFunction& funPtrRef)
{
  switch (receivePhase) {
  case 0 : funPtrRef = &Communicator::allToAll; break;
  case 1 :
...
void* TouchDetector::getRecvbuf(int receiveCycle, int receivePhase)
{
  ...
  case 1 : 
    if (_receiveAtBufferOffset)  rval = (void*)( (*_capsules) + _numberOfCapsules );
    else rval = (void*)(*_capsules);
    break;
  default : assert(0);
  }
  return rval;
...
void TouchDetector::prepareToSend(int sendCycle, int sendPhase, CommunicatorFunction& funPtrRef)
{
  assert(sendCycle==0);
  switch (sendPhase) {
  case 0 :
...
{  
  ...
  void* rval;
  switch (sendPhase) {
  case 0 : rval = (void*)_touchesPerReceiver; break;
#ifdef A2AW
  case 1 : rval = (void*)_touchOrigin; break;
#else
  case 1 : rval = (void*)_sendBuf; break;
#endif
  default : assert(0);
  }
  return rval;
...
int* TouchDetector::getSendcounts(int sendCycle, int sendPhase)
{
  ...
  case 0 : rval = &_numberOfInts; break;
#ifdef A2AW
  case 1 : rval = _touchCounts; break;
#else
  case 1 : rval = _touchesPerReceiver; break;
#endif
  default : assert(0);
  }
  return rval;
...
{  
  ...
  int size=0;
  std::map<int, std::list<TouchIndex> >::iterator mapIter, mapEnd=touchMap.end();
  for (int i=0; i<_numberOfReceivers; ++i) {
    mapIter=touchMap.find(i);
    if (mapIter==mapEnd) _touchesPerReceiver[i]=0;
    else _touchesPerReceiver[i]=mapIter->second.size();
#ifdef A2AW
...
void TouchDetector::initializePhase1Send()
{
  ...
  for (int i=0; i<_numberOfReceivers; ++i) {
    ...
    int j=0;
    if (mapIter!=mapEnd) {
      std::list<TouchIndex>::iterator iter=mapIter->second.begin(), end=mapIter->second.end();
      while (iter!=end) {	
    ...
    MPI_Type_create_hindexed(j, _touchBlockLengths, _touchBlockDisplacements, *(Touch::getTypeTouch()), &_typeTouches[i]);
    MPI_Type_commit(&_typeTouches[i]);
  }
#else
  int writePos=0;
  for (int i=0; i<_numberOfReceivers; ++i) {
	...
	Touch& t = _touchVector->getValue(*iter);
	memcpy(&_sendBuf[writePos], t.getTouchData(), N_TOUCH_DATA*sizeof(double));
...
{
  if (_tissueContext) {
    _numberOfCapsules =_segmentDataSize=_tissueContext->setUpCapsules(_segmentDataSize, _detectionPass, _rank, _maxComputeOrder);
  }
  else {
...
{
#ifdef USING_BLUEGENE
  if (_rank==0) printf("Available memory before touch detection %s : %lf MB.\n\n", getPassName().c_str(), AvailableMemory());
#endif 
  if (_unique) assert((*_decomposition)->isCoordinatesBased());
  _touchVector->clear();
  RNG* rng;
  if (_tissueContext) rng=&_tissueContext->_touchSampler;
  else {
    rng=new RNG;
...
#ifndef DISABLE_PTHREADS
  For<TouchDetector, ThreadUserData>::execute(0, 1, _numberOfCapsules, this, _threadUserData, _nThreads);  
#else
  ...
  if (_tissueContext==0) {
    ...
    for (int sid=0; sid<_numberOfCapsules; ++sid) capsuleMap[caps[sid].getKey()]=sid;
	...
	if (_communicateTouchSpace->isInSpace(s1Key))
	  _touchVector->mapTouch(_neuronPartitioner->getRank(caps[capsuleMap[s1Key]].getSphere()), titer);
	double s2Key = titer->getKey2();
	if (_communicateTouchSpace->isInSpace(s2Key))
	  _touchVector->mapTouch(_neuronPartitioner->getRank(caps[capsuleMap[s2Key]].getSphere()), titer);
      }
      if (_touchAnalyzer!=0) {
  ...
  MPI_Allreduce((void*) &_connectionCount, (void*) &totalCount, 1, MPI_LONG_LONG, MPI_SUM, MPI_COMM_WORLD);
...
void TouchDetector::doWork(int threadID, int sid, ThreadUserData* data, Mutex* mutex) 
{
  ...
  Capsule* caps = *_capsules;
  double s1Ax=0, s1Ay=0, s1Az=0, s1Bx=0, s1By=0, s1Bz=0, s1Ar=0, s1Ab=0,
    s2Ax=0, s2Ay=0, s2Az=0, s2Bx=0, s2By=0, s2Bz=0, s2Ar=0, s2Ab=0,
    a=0,b=0,c=0,d=0,e=0,D=0,
    sc=0,sN=0,sD=0,tc=0,tN=0,tD=0,
    u0=0,u1=0,u2=0,
    v0=0,v1=0,v2=0,
    w0=0,w1=0,w2=0,
    aw0=0,aw1=0,aw2=0,
    bw0=0,bw1=0,bw2=0,
    ww0=0,ww1=0,ww2=0,
    disSphere=0, mdis=0, mdis2=0,
    dist, crit, c1, c2;
  double pointLineDistances[4];
  for(int sid2=0; sid2<_numberOfCapsules; ++sid2) if ( _appositionRate>=1.0 || drandom(rng)<_appositionRate) {
    ...
    s1Ab = sqrt(SqDist(s1begin, s1end))+2.0*s1Ar;     // FOR USE BELOW : OPTIMIZATION
    ...
    if (detectionTouchSpace->areInSpace(s1Key, s2Key)) {
      ...
      //s1Ab = SqDist(s1, &s1[TD_END_COORDS])+2.0*s1Ar; COMPUTED ABOVE : OPTIMIZATION
      ...
      if( ( (ww0 * ww0 + ww1 * ww1 + ww2 * ww2) <= disSphere) ) {  // it is 0.5*0.5
	...
	if (fabs(sN) < SMALL_NUM) aw0=aw1=aw2=sc=0.0;
	else {
	  sc = sN / sD;
	  aw0 = u0 * sc;
	  aw1 = u1 * sc;
	...
	if (fabs(tN) < SMALL_NUM) bw0=bw1=bw2=tc=0.0;
	else {
	  tc = tN / tD;
	  bw0 = v0 * tc;
	  bw1 = v1 * tc;
	...
	if ( dist < crit ) {
	  /**
	   * The coordinates of the touch can be calculated by vector arithmetic as follows:
	  ...
	  if (_unique) {	      
	    ...
	    decomposition->getRanks(&caps[sid].getSphere(), caps[sid].getEndCoordinates(), params->getRadius(s1Key), ranks1);
	    decomposition->getRanks(&caps[sid2].getSphere(), caps[sid2].getEndCoordinates(), params->getRadius(s2Key), ranks2);
    	    ranks1.merge(ranks2);

	    countTouch=false;
	    ShallowArray<int, MAXRETURNRANKS, 100>::iterator ranksIter=ranks1.begin(), ranksEnd=ranks1.end();
	    ...
	    if (!countTouch) {
	      ...
	      countTouch = ( decomposition->getRank(sphere2) == _rank &&
			     !decomposition->mapsToRank(&caps[sid2].getSphere(), 
							s2end, params->getRadius(s2Key), 
							touchVolume) );
	    }
#endif
	  }
	  if (countTouch) {
	    ...
	    distancePointLine(s1begin, s1end, s2begin, s2end, pointLineDistances);
...
void TouchDetector::distancePointLine(double p1A[3], double p1B[3], double p2A[3], double p2B[3], double* pointLineDistances)
{
  ...
  for (int i=0; i<4; ++i) {
    switch (i) {						
    case 0 : p[0]=p1A; p[1]=p2A; p[2]=p2B; break;
    case 1 : p[0]=p1B; p[1]=p2A; p[2]=p2B; break;
    case 2 : p[0]=p2A; p[1]=p1A; p[2]=p1B; break;
    case 3 : p[0]=p2B; p[1]=p1A; p[2]=p1B; break;
    default : break;
    }
...
void TouchDetector::unique(bool unique)
{
  _unique=unique;
}
...
{
  std::string rval;
  if (_detectionPass==TissueContext::FIRST_PASS) rval="first pass";
  else if (_detectionPass==TissueContext::SECOND_PASS) rval="second pass";
  else rval = "additional pass";
  return rval;
}
