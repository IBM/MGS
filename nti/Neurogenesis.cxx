...
Neurogenesis::Neurogenesis(int rank, int size, int nThreads, std::string statsFileName, 
  ...
  : NeuronBoundary(0),
    ...
    _threadData(0)
{
  WaypointGeneratorMap["OlivoCerebellar"]=new OlivoCerebellarWaypointGenerator();
  WaypointGeneratorMap["Test"]=new TestWaypointGenerator();
  NeuronBoundary = new BoundingCuboid();
  _threadData = new ThreadData[_nThreads];
}

void Neurogenesis::run(int neuronBegin, int nNeurons, NeurogenParams** params, char** fileNames, bool* somaGenerated)
{
  std::map<std::string, WaypointGenerator*>::iterator miter, mend=WaypointGeneratorMap.end();
...
#ifndef DISABLE_PTHREADS
    For<Neurogenesis, ThreadData>::execute(0, 1, nNeurons, this, _threadData, _nThreads);  
#else
...
void Neurogenesis::doWork(int threadID, int nid, ThreadData* data, Mutex* mutex)
{
...
void Neurogenesis::readSoma(int threadID, int nid, int& count, NeurogenParams* params_p)
{
  ...
  while (fscanf(inputDataFile, "%d %d %lf %lf %lf %lf %d", &id, &type, &x, &y, &z, &r, &parent)!=EOF) {
    if (type==1) {
      if (first) {
	first->setRadius(r);
	first->setID(count);
	...
	NeurogenSegment* seg_p = data.allocSegment(count, 1, x, y, z, r, 1, params_p);
...
void Neurogenesis::generateSoma(int threadID, NeurogenParams* params_p, int& count)
{
  ThreadData& data = _threadData[threadID];
  ...
  for (int i=0; i<params_p->somaSegments; i++) {
    NeurogenSegment* seg_p = data.allocSegment(i+2, 1, params_p->startX, params_p->startY+i*somaRadius, params_p->startZ, somaRadius, 1, params_p);
    seg_p->setParent(&data.Segments[0]);
    seg_p->setID(count);
    ++count;
    soma->addSegment(seg_p);
...
void Neurogenesis::generateStems(int threadID, NeurogenParams* params_p, int nid, int& totalBifurcations, int& nrStemSegments)
{
  ...
  int nrStems=0;  
  if (_boundingSurfaceMap[params_p->boundingSurface]->isOutsideVolume(&data.Segments[0])) {
    std::cerr<<"Neuron "<<data.nid<<", ("<<data.Segments[0].getX()<<", "<<
      data.Segments[0].getY()<<", "<<data.Segments[0].getZ()<<"), is outside surface_mesh.vtk!"<<std::endl;
    exit(0);
  }
  ...
  while (nrStems<params_p->nrStems) {
    ...
    if (nrStems==0)
      ...
      newSeg = data.allocSegment(_branchType, params_p->startX, params_p->startY+1.0, params_p->startZ,
				 params_p->startRadius, params_p);
    else
#endif
      // Create Neurite in Random direction
      newSeg = data.allocSegment(_branchType, params_p->getGaussian(data.Segments[0].getX(), 1.0),
    ...
    int resampleCount = 0;
    // simple rejection sampling to make sure that the dendrite stems are not too close to each other
    int resamples=0;

    while (resamples<360 && (withinMinimumStemAngleOfAnotherSeg(threadID, newSeg, double(360-resamples)/360.0, params_p) || touchesAnotherSeg(threadID, newSeg, params_p) ) ) {
    ...
    if (attempts>=1000) {
      std::cerr<<"ERROR: Only "<<nrStems<<" type "<<_branchType<<" stems created!"<<std::endl;
      exit(0);
    }

    WaypointGenerator* waypointGenerator=WaypointGeneratorMap[params_p->waypointGenerator];
	...
	if (i<nrStems) origins[i]=&data.Segments[data.nrSegments-nrStems+i];
	else origins[i]=0;
      }
      bool done=false;
      while (!done) {
	...
	for (int i=0; i<nrTerminals; ++i) {
	  if (waypointCoords[i].size()>0) {
		...
		if (waypointCoords[j].size()>0) {
		  branchCandidates[origins[j]->getDistance(waypointCoords[i])]=j;
		  origins[j]->getBranch()->addWaypoint(waypointCoords[i]);
		}
	      }
	    }
	  }
	  else if (origins[i]!=0) ++nrDoneGrowing;
	}

	done=(nrDoneGrowing==nrTerminals);
	if (branchCandidates.size()>0) {
	  int branch1, branch2=nrGrowingStems;
	  std::map<double, int>::iterator branchCandidate=branchCandidates.begin(),
	    candidatesEnd=branchCandidates.end();
	  bool success=false;
	  ...
	  if (!success) {
	    std::cerr<<"Neurogenesis : Branching failed on rank "<<params_p->_rank<<" during waypoint guided stem extension of neuron "<<nid<<"!"<<std::endl;
	    MPI_Finalize();
	    exit(0);
	  }
	  nrStemSegments+=2;
	...
	for (int i=0; i<nrGrowingStems; ++i) {
	  if (waypointCoords[i].size()>0) {
	    ...
	    while (dist>params_p->waypointExtent && growSegment(threadID, tip, params_p)) {
	      nrStemSegments++;
	...
	waypointGenerator->next(waypointCoords, nid);
	for (int i=0; i<nrGrowingStems; ++i) origins[i]->getBranch()->clearWaypoints();
      }
...
#ifdef CONST_BR_PROB
    double branchProb = 1.0 - pow(1.0-params_p.branchProb, data.Segments[j].getLength());
#else
...
bool Neurogenesis::branchWithAngle(int threadID, NeurogenSegment* seg_p, int& totalBifurcations, NeurogenParams* params_p)
{
  ...
  double fac = pow(params_p->minRadius/newRadius1, params_p->branchDiameterAsymmetry);
  
  if (fac<1.0) {
    double fac2 = params_p->radiusRate * (1.0 - fac) + 0.5 * params_p->RallsRatio * fac;
  ...
  NeurogenSegment* child1 = data.allocSegment(seg_p->getType(), seg_p->getX(), seg_p->getY(), seg_p->getZ(), 
						newRadius1, params_p);
  child1->setParent(seg_p);
  child1->growSameDirectionAsParent();

  NeurogenSegment* child2 = data.allocSegment(seg_p->getType(), seg_p->getX(), seg_p->getY(), seg_p->getZ(), 
						newRadius2, params_p);
  ...
  double lFront1 = params_p->umnPerFront / pow(2.0*M_PI*child1->getRadius(), params_p->nexpPerFront);;
  child1->setLength(lFront1);
  double lFront2 = params_p->umnPerFront / pow(2.0*M_PI*child2->getRadius(), params_p->nexpPerFront);;
  child2->setLength(lFront2);

  if (params_p->genZ>0.5) seg_p->rotateDaughters(child1, child2);
  else seg_p->rotateDaughters2D(child1, child2);
  ...
  if (checkTouchAndRelax(threadID, child1, params_p) ) {
    ...
    std::cout << "Warning! Daughter Branch touching other segments! Terminating Branch!" << std::endl;
#endif
    branch1=0;
  } 
...
bool Neurogenesis::growSegment(int threadID, NeurogenSegment* seg_p, NeurogenParams* params_p)
{
  bool rval=false;
  ThreadData& data = _threadData[threadID];
  NeurogenSegment* newSeg = data.allocSegment(seg_p->getType(), seg_p->getX(), seg_p->getY(), seg_p->getZ(), seg_p->getRadius(), params_p);
  double newRadius = seg_p->getRadius() * params_p->radiusRate;
  double lFront = params_p->umnPerFront / pow(2.0*M_PI*newRadius, params_p->nexpPerFront);
  ...
  if (checkTouchAndRelax(threadID, newSeg, params_p)) {
    data.deleteSegment();
#ifdef SHOWTERMINATIONS
    std::cout << "Warning! Growing Segment touching other segments! Terminating Branch!" << std::endl;
#endif
  }
    ...
    data.deleteSegment();
#ifdef SHOWTERMINATIONS
    std::cout << "Warning! Growing Segment intersects with another segment!  Terminating Branch! " << std::endl;
#endif
  }
  else {
    rval = true;
...
#ifdef DBG
  //	std::cout << "Updated Grown Distance: " << grownDistance << " and lFront: " << lFront << std::endl;
...
bool Neurogenesis::touchesAnotherSeg(int threadID, NeurogenSegment* seg_p, NeurogenParams* params_p)
{
  ThreadData& data = _threadData[threadID];
  bool rval=false;
  for (unsigned int i=1+params_p->somaSegments; i<data.nrSegments && !rval; i++) {
...
bool Neurogenesis::touchesAnotherSegWithExceptions(int threadID, NeurogenSegment* seg_p, NeurogenParams* params_p)
{
  ThreadData& data = _threadData[threadID];
  bool rval=false;
  for (unsigned int i=1+params_p->somaSegments; i<data.nrSegments && !rval; i++) {
    ...
    else if (seg_p->getParentSeg()->getParentSeg() && seg_p->getParentSeg()->getParentSeg()->equals(&data.Segments[i])) {}
    //std::cout << "Skipping grand parent" << std::endl;
    else if (seg_p->touches(&data.Segments[i], params_p->intolerance)) {
#ifdef DBG
      std::cout << "Segment: " << std::endl << seg_p->outputLine() << " touches " << data.Segments[i].outputLine() << std::endl;
#endif
      rval=true;
    }
  }
...
bool Neurogenesis::withinMinimumStemAngleOfAnotherSeg(int threadID, NeurogenSegment* seg_p, double factor, NeurogenParams* params_p)
{
  ThreadData& data = _threadData[threadID];
  bool rval=false;
  for (unsigned int i=1+params_p->somaSegments; i<data.nrSegments && !rval; i++) {
    if (seg_p==&data.Segments[i]) continue;
    if (seg_p->getAngle(&data.Segments[i])<M_PI/180.0*params_p->minInitialStemAngle*factor) {
      rval=true;
    }
  }
  return rval;
...
void Neurogenesis::segmentsNear(int threadID, ShallowArray<NeurogenSegment*>& SegTouchVec, NeurogenSegment* seg_p, NeurogenParams* params_p)
{
  ThreadData& data = _threadData[threadID];
...
#ifdef DBG
    std::cout << "Checking for touches " << std::endl;
    if (seg_p->getParentSeg()->equals(&data.Segments[i]) ) std::cout << "Skipping own parent" << std::endl;
#endif
    if (!seg_p->getParentSeg()->equals(&data.Segments[i]) && !seg_p->getParentSeg()->getParentSeg()->equals(&data.Segments[i])) {
#ifdef DBG
      std::cout << "NEWSEG: " << seg_p->outputLine() << " is within the homotypic distance " << data.Segments[i].outputLine() << std::endl;
      std::cout << " NewSeg's parent was " << seg_p->getParentSeg()->outputLine() << std::endl;
#endif
...
{
  bool rval=false;
  if(NeuronBoundary->isOutsideVolume(newSeg) || _boundingSurfaceMap[newSeg->getParams()->boundingSurface]->isOutsideVolume(newSeg) ) {
    rval=true;
  } 
  return rval;
}

bool Neurogenesis::checkRadius(NeurogenSegment* newSeg, NeurogenParams* params_p)
{
  bool rval=false;
...
bool Neurogenesis::checkIntersect2D(int threadID, NeurogenSegment* newSeg)
{
  ...
  for (int i=1; i<data.nrSegments && !rval; i++) {
    ...
    //std::cout << "Skipping own parent from intersection check" << std::endl;
    else if (newSeg->intersects(&data.Segments[i])) rval=true;
  }
  return rval;
}

bool Neurogenesis::checkTouchAndRelax(int threadID, NeurogenSegment* newSeg, NeurogenParams* params_p)
{
  bool rval=false;
  int resampleCount = 0;
...
#ifdef DBG
      std::cout << "newSeg " << newSeg->outputLine() <<  " touches some other segment!!!!!"<<std::endl;
#endif
      newSeg->resampleAfterForces(params_p->gaussSD*(1.0+(MAX_RELAX_MULTIPLE-1.0)*resampleCount/params_p->maxResamples) );
      newSeg->setLength(params_p->umnPerFront / pow(2.0*M_PI*newSeg->getRadius(), params_p->nexpPerFront));
      resampleCount++;
    }
    else rval=true;
  }
...
void Neurogenesis::applyForces(int threadID, NeurogenSegment* newSeg, ShallowArray<NeurogenSegment*>& SegTouchVec, NeurogenParams* params_p)
{
  ...
  std::cout << newSeg->getBiasX() << "\t" << newSeg->getBiasY() << "\t" < newSeg->getBiasZ() << std::endl;
#endif
  newSeg->forwardBias();
#ifdef DBGFORCES
...
void Neurogenesis::resampleAfterForces(NeurogenSegment* newSeg)
{
  ...
  //std::cout << "Current Segment after all Forces: " << newSeg->outputLine()  << std::endl;
  newSeg->setLength(lFront);
}
 
void Neurogenesis::setSegmentIDs(int threadID, NeurogenParams* params_p, int& count)
{
  ThreadData& data = _threadData[threadID];
...
double Neurogenesis::getTotalFiberLength(int threadID, NeurogenParams* params_p)
{
  ThreadData& data = _threadData[threadID];
...
{
  ThreadData& data = _threadData[threadID];
  double totalArea = 4.0*M_PI*data.Segments[0].getRadius()*data.Segments[0].getRadius();
  for (unsigned int i=1+params_p->somaSegments; i<data.nrSegments; i++) {
...
{
  ThreadData& data = _threadData[threadID];
  double totalVol = 1.3333333333333*M_PI*data.Segments[0].getRadius()*data.Segments[0].getRadius()*data.Segments[0].getRadius();
  for (unsigned int i=1+params_p->somaSegments; i<data.nrSegments; i++) {
...
double Neurogenesis::getAvgBifurcationAngle(int threadID)
{
  ...
  for (unsigned int i=1; i<data.nrBranches; i++) {
    for (int j=i+1; j<data.nrBranches; j++) {
      if (data.Branches[i].getFirstSegment()->getParent()==data.Branches[j].getFirstSegment()->getParent() 
	  && data.Branches[i].getFirstSegment()->getParentSeg()->getType()>1) {
	sumAngles += data.Branches[i].getFirstSegment()->getAngle(data.Branches[j].getFirstSegment());
	angleCount++;
      }
    }   
  }
...
int Neurogenesis::getMaxBranchOrder(int threadID)
{
  ...
  for (unsigned int i=0; i<data.nrBranches; i++) {
    int branchOrder=1;
    NeurogenSegment* parent=data.Branches[i].getFirstSegment()->getParentSeg();
    while (parent!=0) {
...
void Neurogenesis::writeSWC(int threadID, int nid, const char* fileName)
{
  ...
  std::ofstream fout;
  if (_somaGenerated[nid]) fout.open(fileName, std::ios::app);
  else fout.open(fileName);
  for (unsigned int i=(_somaGenerated[nid] ? 1:0); i<data.nrBranches; i++) {
...
// obsolete, only prints out SWC file by Segment rather than by normal branch order
...
Neurogenesis::~Neurogenesis()
{
  ...
  delete [] _threadData;
  for (std::map<std::string, WaypointGenerator*>::iterator iter=WaypointGeneratorMap.begin();
...
NeurogenSegment* Neurogenesis::ThreadData::allocSegment(NeurogenParams* params)
{
  NeurogenSegment* rval=&Segments[nrSegments];
  ++nrSegments;
  rval->set(params);
...
NeurogenSegment* Neurogenesis::ThreadData::allocSegment(int id, int type, double x, double y, double z, double r, int p, NeurogenParams* params)
{
  NeurogenSegment* rval=&Segments[nrSegments];
  ++nrSegments;
...
NeurogenSegment* Neurogenesis::ThreadData::allocSegment(int type, double x, double y, double z, double r, NeurogenParams* params)
{
  NeurogenSegment* rval=&Segments[nrSegments];
  ++nrSegments;
...
void Neurogenesis::ThreadData::deleteSegment()
{
  --nrSegments;
}
