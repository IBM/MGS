...
class Params
{
  ...
  bool symmetricBidirectionalConnectionTargets(key_size_t key1,
                                               key_size_t key2);
  ...
  std::list<ElectricalSynapseTarget>* getElectricalSynapseTargets(
      key_size_t key1, key_size_t key2);
  std::list<BidirectionalConnectionTarget>* getBidirectionalConnectionTargets(
      key_size_t key1, key_size_t key2);
  ...
  bool isNonTouchableTargets(key_size_t key1, key_size_t key2);
  bool isCompartmentVariableTarget(key_size_t key, std::string type);
  bool isChannelTarget(key_size_t key);
  bool isElectricalSynapseTarget(key_size_t key1, key_size_t key2,
                                 bool autapses);
  ...
  bool isBidirectionalConnectionTarget(key_size_t key1, key_size_t key2,
                                       bool autapses);
  ...
  void readMarkovModel(const std::string& fname,
                       dyn_var_t**& matChannelRateConstant, int& numChanStates,
                       int*& vChannelStates, int& initialstate);
  void readMarkovModel(const std::string& fname,
                       dyn_var_t*& matChannelRateConstant, int& numChanStates,
                       int*& vChannelStates, int& initialstate);
  void setupCluster(
      dyn_var_t* const& matChannelRateConstant, const int& numChan,
      const int& numChanStates, int* const& vChannelStates,
      // output
      int& numClusterStates, int*& matClusterStateInfo,
      int*& vClusterNumOpenChan, int& maxNumNeighbors,
      // Combined2MarkovState_t * &matK_channelstate_fromto, long* &indxK
      long*& matK_channelstate_fromto, ClusterStateIndex_t*& indxK);
  void getCompactK(dyn_var_t* const& matChannelRateConstant,
                   const int& numChanStates, const int& numChan,
                   int* const& matClusterStateInfo, const int& numClusterStates,
                   // output
                   int& maxNumNeighbors, long*& matK_channelstate_fromto,
                   // int* & matK_indx
  ...
  void buildCompartmentVariableTargetsMap(
      std::vector<SegmentDescriptor::SegmentKeyData>& maskVector,
      std::vector<unsigned int*>& v_ids, const std::string& myBuf);
  void buildRadiiTargetsMap(
      std::vector<SegmentDescriptor::SegmentKeyData>& maskVector,
      std::vector<unsigned int*>& v_ids, const std::string& myBuf);
  void buildParamsMap(
      std::vector<SegmentDescriptor::SegmentKeyData>& maskVector,
      std::vector<unsigned int*>& v_ids,
      // std::istringstream& is,
      const std::string& myBuf, std::string& modelID,
      std::map<
          std::string,
  ...
  void buildBidirectionalConnectionMap(
      ...
      std::vector<unsigned int*>& v1_ids, std::vector<unsigned int*>& v2_ids,
  ...
  void buildChemicalSynapseConnectionMap(
      ...
      std::vector<unsigned int*>& v1_ids, std::vector<unsigned int*>& v2_ids,
      const std::string& myBuf);
  void buildNonTouchableTargetsVector(
      std::vector<SegmentDescriptor::SegmentKeyData>& maskVector1,
      std::vector<SegmentDescriptor::SegmentKeyData>& maskVector2,
      std::vector<unsigned int*>& v1_ids, std::vector<unsigned int*>& v2_ids);
  ...
  ErrorCode readRadii2(FILE* fpF);
  bool readTouchTables(FILE* fpF);
  bool readSIParams(FILE* fpF);
  bool readCompartmentVariableTargets(FILE* fpF);  // support array-form only
                                                   // BRANCHTYPE
  bool readCompartmentVariableTargets2(FILE* fpF);  // support array-form for
                                                    // any
  ...
  ErrorCode readElectricalSynapseTargets_vector2(FILE* fpF);  // support
                                                              // array-form for
                                                              // all
  bool readBidirectionalConnectionTargets(FILE* fpF);
  bool readBidirectionalConnectionTargets_vector(FILE* fpF);  // array-form for
                                                              // only BRANCHTYPE
  ErrorCode readBidirectionalConnectionTargets_vector2(
      FILE* fpF);  // support array-form for all
  bool readChemicalSynapseTargets(FILE* fpF);
  ErrorCode readChemicalSynapseTargets_vector2(FILE* fpF);  // support
                                                            // array-form for
                                                            // all
  ...
  unsigned long long _radiiMask, _SIParamsMask, _compartmentVariableTargetsMask,
      _channelTargetsMask, 
      _electricalSynapseTargetsMask1, _electricalSynapseTargetsMask2, 
      _nonTouchableTargetsMask1,_nonTouchableTargetsMask2,
      _chemicalSynapseTargetsMask1, _chemicalSynapseTargetsMask2;
  ...
  //      the 2nd arg is a map containing the associated parameters
  //      (Epsilon,Sigma)
  ...
  //      each channel/receptor is represented as an instance of 'ChannelTarget'
  //      class
  //      ChannelTarget class keeps
  //         1. the names of the node
  //         2. the list of names of nodes as providing input to the
  //         channel/receptor
  //         3. the list of names of nodes as expecting to receive the output
  //         from the channel/receptor
...
//       containing a list of pairs (param,value)
  ...
  //      each bidirectional connection is represented 'ElectricalSynapseTarget'
  //      class
  //      ElectricalSynapseTarget class keeps
  //         1. the names of the node (as the value to nodekind in Layer
  //         statement)
  //         2. the list of names of diffusible nodes whose data
  //             will flow in 2 directions
  //         3. a double _parameter = -1.0 (default) representing the prob. for
  //         forming the
  //                                   bidirectional connection (e.g. electrical
  //                                   synapse
  //                                      or spine_neck + branch's compartment)
  std::map<key_size_t,
           std::map<key_size_t, std::list<ElectricalSynapseTarget> > >
      _electricalSynapseTargetsMap;
  std::map<key_size_t,
           std::map<key_size_t, std::list<BidirectionalConnectionTarget> > >
      _bidirectionalConnectionTargetsMap;
  std::set<std::pair< key_size_t, key_size_t > >
      _nonTouchableTargetsVector;
  // define mapping a compartment of a given key (1st arg)
  //    to a second compartment of a given key (1st arg_level2)
  //      each chemicalsynapse is represented as a list of
  //      'ChemicalSynapseTarget'
  //      an instance of ChemicalSynapseTarget contains
  //          1. a double _parameter = -1.0 (default) representing the prob. for
  //          forming the
  //                                   chemical synapse
  //          2. the map of (names of receptor/channels nodes, a pair of
  //                          2 list of names
  //                           list 1 = names of nodes as providing input to the
  //                           channel
  //                           list 2 = name sof nodes as receiving the output
  //                           from the channel
  ...
  //      _channels == TRUE when CHEMICAL_SYNAPSE_TARGETS section in
  //      SynParams.par is read
  bool _SIParams, _compartmentVariables, _channels, _electricalSynapses,
       _nonTouchableTargets,
       _chemicalSynapses;
  ...
  void buildSpinesMap(
      std::vector<SegmentDescriptor::SegmentKeyData>& maskVector,
      std::vector<unsigned int*>& v_ids, std::vector<key_size_t>& mymap
      // std::istringstream &is
      // const std::string & myBuf
      );
#endif
  ...
  static void reviseParamValue(unsigned int& fieldVal,
                               const std::string& fieldName);
  static void reviseParamValues(std::vector<int>& fieldVals,
                                const int& fieldIdx);
  static void reviseParamValues(std::vector<int>& fieldVals,
                                const std::string& fieldName);
