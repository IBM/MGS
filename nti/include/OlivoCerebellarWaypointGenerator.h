...
public:
  ...
  int _nrNeurons;
  ShallowArray<ShallowArray<tract_t, MAX_MULTITHREADED_TRACTS, 4>,
               MAX_MULTITHREADED_NEURONS, 4> _allNeuronTracts;
  ShallowArray<ShallowArray<int, MAX_MULTITHREADED_TRACTS, 4>,
               MAX_MULTITHREADED_NEURONS, 4> _pointIndexPerNeuronTract;
  ...
  void readWaypoints(std::vector<std::string>& fileNames, int nrNeurons) {
    ...
    for (int nid = 0; nid < nrNeurons; ++nid) {
      ShallowArray<tract_t, MAX_MULTITHREADED_TRACTS, 4>& tractsVec =
          _allNeuronTracts[nid];
      ...
      if (tractFile.is_open()) {
        while (tractFile.good() && getline(tractFile, nextLine)) {
          if (nextLine != "" && nextLine[0] != '#') {
            if (nextLine.find('.') == std::string::npos) {  // an integer
                                                            // providing the
                                                            // number of points
              if (!tractTerminated)
                tractsVec[nr_tracts - 1].push_back(lastPoint);
              ++nr_tracts;
              tract_t nextTract;
              ...
              // Add point to a new tract, else add it to the last tract but not
      ...
      if (nr_tracts > 0) {
        ...
        for (int i = 1; i < nr_tracts; ++i) {
          _pointIndexPerNeuronTract[nid][i] =
              _pointIndexPerNeuronTract[nid][i - 1] + 1 +
              int((i == 1 ? 0.25 : 0.02) * (double(tractsVec[i].size())));
  ...
  void next(ShallowArray<ShallowArray<double> >& waypointCoords, int nid) {
    ...
    if (_nextPerNeuron[nid] < _maxTractPointsPerNeuron[nid]) {
      for (int tractIndex = 0; tractIndex < ntracts; ++tractIndex) {
        ...
        if (_nextPerNeuron[nid] >= nextPointIndex &&
            nextPointIndex < tractsVec[tractIndex].size()) {
          double* cds = tractsVec[tractIndex][nextPointIndex].coords;
          for (int i = 0; i < 3; ++i)
            waypointCoords[tractIndex].push_back(cds[i]);
          /*
          int rank;
