...
Tissue::Tissue(int size, int rank, bool logTranslationHistory, bool logRotationHistory)
  : _neuronArraySize(0),
    _segmentArraySize(0),
    _neuronIndex(0),
    _neurons(0),
...
void Tissue::loadBinary(FILE* inputDataFile,
			char* inputFilename,
			const int neuronArraySize,
			...
			double pointSpacing)
{
  ...
  for(int i = 0; i<neuronArraySize; i++) {
    ...
    segmentPtr = _neurons[i].loadBinary(inputDataFile, segmentPtr, this, i, 
				  _neuronTranslationFile, _neuronRotationFile);
...
void Tissue::loadText(char * inputFilenames,
		      const int neuronArraySize,
		      const int segmentArraySize,
		      const int startNeuron,
		      NeuronPartitioner* neuronPartitioner,
		      ...
		      double pointSpacing)
{
  ...
  if((filenameFile = fopen(inputFilenames, "rt")) == 0) {
  ...
  for (int i=0; i<startNeuron; ++i) {
    strcpy(bufS,"");
    do {c=fgets(bufS,1024,filenameFile);} while (bufS[0]=='#' || bufS[0]=='\n');
    if (11==sscanf(bufS, "%s %d %d %d %lf %lf %lf %c %s %s %s", filename, &layer, &morphtype, &electrotype, &x, &y, &z, &offsetType, axonPar, basalPar, apicalPar)) {
      std::string fname(filename);
      filenameMapIter=filenameMap.find(fname);
      if (filenameMapIter==filenameMap.end()) filenameMap[fname]=0;
      else ++filenameMapIter->second;
    }
  }
  ...
  for (int i=0; i<_neuronArraySize; ++i)  {
	...
	exit(0);
      ...
      segmentPtr = _neurons[i].loadText(inputDataFile, segmentPtr, this, i,
					_neuronTranslationFile, _neuronRotationFile, layer,
					morphtype, electrotype, x, y, z, offsetType);
      fclose(inputDataFile);
    }
  }
  if(segmentPtr!=_segments+_segmentArraySize) {std::cerr<<segmentPtr-(_segments+_segmentArraySize)<<std::endl; assert(0);}
  fclose(filenameFile);
...
void Tissue::resampleNeurons(const int neuronArraySize, std::vector<Segment>& segments, double pointSpacing)
{
  assert(!isEmpty());
  for (int i=0; i<neuronArraySize; ++i) _neurons[i].resample(segments, pointSpacing);
  resetSegments(segments, true);
  for (int i=0; i<neuronArraySize; ++i) _neurons[i].eliminateLostBranches();
}

void Tissue::resetBranchRoots(const int neuronArraySize, std::vector<Segment>& segments)
{
  assert(!isEmpty());
  for (int i=0; i<neuronArraySize; ++i) _neurons[i].resetBranchRoots(segments);
...
void Tissue::updateCellBodies()
{
  for (int i=0; i<_segmentArraySize; ++i) {
	...
	_segments[i].getSegmentIndex()==1) {
      memcpy(_segments[i-1].getCoords(), _segments[i].getCoords(), sizeof(double)*3);
    }
  }
}
...
void Tissue::updateBranchRoots(int frontNumber)
{
  if (frontNumber!=0) {
	...
	assert(_segments[i].getFrontLevel()==_segments[i].getBranch()->getRootSegment()->getFrontLevel());
...
void Tissue::resetSegments(std::vector<Segment>& segments, bool resampled)
{
  ...
  _segments = new Segment[_segmentArraySize];//Create the new array of segments
  ...
  for (int i=0; i<_segmentArraySize; ++i) {
	...
	thisBranch->resetSegments(&_segments[i], &_segments[thisBranch->getRootSegment()->getBranch()->getResampledTerminalIndex()]);
      }
      else thisBranch->resetSegments(&_segments[i]);
...
{
#ifdef BINARY64BITS
  fseeko64(outputDataFile, bin_start_pos + NEURON_COUNT_SIZE + sizeof(int)*_neuronIndex, SEEK_SET);
#else
  fseek(outputDataFile, bin_start_pos + NEURON_COUNT_SIZE + sizeof(int)*_neuronIndex, SEEK_SET);
#endif
  for (int i=0; i<_neuronArraySize; ++i) {
...
void Tissue::writeCoordinates(FILE* outputDataFile, PosType bin_start_pos)
{
  ...
  for (int i=0; i<_neuronArraySize; ++i) {
    ...
    fseek(outputDataFile, bin_start_pos + NEURON_COUNT_SIZE + 
	  sizeof(int)*_neuronPartitioner->getTotalNeurons() + 
	  sizeof(PosType)*(_neuronIndex+i), SEEK_SET);
...
void Tissue::generateBins(double*& columnSizeXYZ, int*& nbinsXYZ, double*& binwidth, double*& maxXYZ, double*& minXYZ)
{
 double localMaxXYZ[3]={INT_MIN,INT_MIN,INT_MIN}, localMinXYZ[3]={INT_MAX,INT_MAX,INT_MAX};
  for (int i=0; i<_segmentArraySize; ++i) {
    for (int d=0; d<3; ++d) {
  ...
  MPI_Allreduce((void*)localMaxXYZ, (void*)_maxXYZ, 3, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);      
  MPI_Allreduce((void*)localMinXYZ, (void*)_minXYZ, 3, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);      
  for (int d=0; d<3; ++d) {
    _columnSizeXYZ[d]=(_maxXYZ[d]-_minXYZ[d]==0) ? MIN_COLUMN_DIM : _maxXYZ[d]-_minXYZ[d];
...
void Tissue::generateHistogram(int& total, int**& histogramXYZ, SegmentSpace* segmentSpace, TouchSpace* touchSpace)
{
  ...
  for (int d=0; d<3; ++d) {
	...
	if ( (segmentSpace?segmentSpace->isInSpace(&_segments[j]):true) &&
	     (touchSpace?touchSpace->isInSpace(_segments[j].getSegmentKey()):true) &&
	     _segments[j].getCoords()[d]>=_minXYZ[d]+(i*_binwidth[d]) &&
	     (i==_nbinsXYZ[d]-1 || _segments[j].getCoords()[d]<_minXYZ[d]+((i+1)*_binwidth[d]))) {
	  ++_localHistogramXYZ[d][i];
	  if (d==0) ++localTotal;
	}
      }
    }
    MPI_Allreduce((void*)_localHistogramXYZ[d], (void*)_histogramXYZ[d], _nbinsXYZ[d], MPI_INT, MPI_SUM, MPI_COMM_WORLD);      
  }
...
void Tissue::generateAlternateHistogram()
{
  ...
  for (int d=0; d<3; ++d) {
    ...
    for (int i=0; i<_nbinsXYZ[d]; ++i) denominator+=double(_histogramXYZ[d][i]);
...
{
  PosType histBytes=sizeof(PosType)+sizeof(int)*(4+_nbinsXYZ[0]+_nbinsXYZ[1]+_nbinsXYZ[2])+sizeof(double)*9;
...
void Tissue::openLogFiles()
{
  if (_logTranslationHistory) {
    char tfname[256];
    sprintf(tfname, "outNeurons_%d-%d_Translations.bin", _neuronIndex, _neuronIndex+_neuronArraySize-1);
  ...
  if (_logRotationHistory) {
    char rfname[256];
    sprintf(rfname, "outNeurons_%d-%d_Rotations.bin", _neuronIndex, _neuronIndex+_neuronArraySize-1);
...
void Tissue::translateNeuron(int neuronIndex, double translation[3], int iteration)
{
  _neurons[neuronIndex].translate(translation, iteration);
}

int Tissue::outputTextNeurons(std::string outExtension, FILE* tissueOutFile, int globalOffset)
{
  ...
  for (int neuronID=0; neuronID<_neuronArraySize; ++neuronID) {
    ...
    segmentsWritten+=outputTextNeuron(neuronID, outName, tissueOutFile, globalOffset+segmentsWritten);
  }
...
int Tissue::outputTextNeuron(int neuronID, std::string outName, FILE* tissueOutFile, int globalOffset)
{
  ...
  for (int i=0; i<_segmentArraySize; ++i) {
	...
	if ( ( outfile = fopen(outName.c_str(), "wt") ) == NULL) {
      ...
      fprintf(outfile, "# output neuron %d, %d, %d, %d\n", n->getNeuronIndex(), i, neuronBegin, neuronEnd);
      for (; i<neuronEnd; ++i) {
	s=&_segments[i];
	b=s->getBranch();
	...
	if (b->getSegments()<s) {
	  if (b->getSegments()==s-1) {
	    ...
	    if (root) rootSegmentIndex=root->getSegmentArrayIndex()-neuronBegin-root->getBranch()->getBranchIndex();
	  }
	  else rootSegmentIndex=segmentIndex-1;
	  fprintf(outfile, "%d %d %lf %lf %lf %lf %d\n", segmentIndex, b->getBranchType()+1,
		 coords[0], coords[1], coords[2], radius, rootSegmentIndex);
	  if (tissueOutFile) {
#ifdef COMPOSITE_OUTPUT
	    fprintf(tissueOutFile, "%d %d %lf %lf %lf %lf %d\n", ++segCount+globalOffset, (b->getBranchType()==0 && n->getGlobalNeuronIndex()>0) ? 2:b->getBranchType()+1,
		 coords[0], coords[1], coords[2], radius, (rootSegmentIndex==-1) ? -1:rootSegmentIndex+globalOffset);
#else
	    if (n!=nPrev) {
	      fprintf(tissueOutFile, "%s %d %d %d %lf %lf %lf %c NULL NULL NULL\n", outName.c_str(), n->getLayer(), n->getMorphologicalType(), n->getElectrophysiologicalType(), 
#ifdef TISSUE_COORD_SWC_OUT
		      0.0, 0.0, 0.0, 'A');
#else
		      n->getCenter()[0], n->getCenter()[1], n->getCenter()[2], 'A');
#endif
	      nPrev=n;
	    }
#endif
...
void Tissue::outputBinaryNeurons(std::string outName)
{
  ...
  if(outName=="NONE") {
	...
	if (_rank>0) {
	  ...
	  fwrite(_neuronPartitioner->getNeuronsPerLayer(), sizeof(int), 6, outputDataFile);
	  fwrite(&totalNeurons, sizeof(int), 1, outputDataFile);	  

	  sz=totalNeurons*sizeof(int);
	  kb=sz/1000;
	  rm=sz%1000;
	  for (PosType i=0; i<kb; ++i) fwrite(kbyte, 1000, 1, outputDataFile); // placeholder for number of segments
	  for (PosType i=0; i<rm; ++i) fwrite(&byte, 1, 1, outputDataFile);

	  sz=totalNeurons*sizeof(PosType);
	  kb=sz/1000;
	  rm=sz%1000;
	  for (PosType i=0; i<kb; ++i) fwrite(kbyte, 1000, 1, outputDataFile); // placeholder for position of segments in file
	  for (PosType i=0; i<rm; ++i) fwrite(&byte, 1, 1, outputDataFile);
...
#endif	  
	  sz=totalNeurons*NEURON_SIZE + getTotalNumberOfBranches()*BRANCH_SIZE + 
	    _neuronPartitioner->getTotalSegmentsRead()*SEGMENT_SIZE;
	  kb=sz/1000;
	  rm=sz%1000;
	  for (PosType i=0; i<kb; ++i) fwrite(kbyte, 1000, 1, outputDataFile);
	  for (PosType i=0; i<rm; ++i) fwrite(&byte, 1, 1, outputDataFile);
	}
	...
	written=1;
      ...
      MPI_Allreduce((void*)&pos, (void*)&nextPos, 1, MPI_POS_TYPE, MPI_MAX, MPI_COMM_WORLD);      
    }
  }
}
...
int Tissue::getTotalNumberOfBranches() 
{
  if (_totalBranches==-1) {
    ...
    MPI_Allreduce((void*)&localTotal, (void*)&_totalBranches, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  }
...
void Tissue::getVisualizationSpheres(SegmentSpace& space, int& nspheres, 
					   float*& positions, float*& radii, int*& types)
{
  nspheres=0;
  for(int i=0; i<_segmentArraySize; ++i)  {
	...
	types=typ;
      ...
      types[idx]=_segmentDescriptor.getValue(_segmentDescriptor.getSegmentKeyData("USER_FIELD_0"), _segments[i].getSegmentKey()) & 0x7;
      idx*=3;
      for (int j=0; j<3; ++j) {
...
void Tissue::getLocalHistogram(int**& histogram, double*& minXYZ, double*& maxXYZ, double*& binwidth, int*& nbinsXYZ)
{
