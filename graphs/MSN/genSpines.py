...
def getNeuronStatisticsFromTissueFile(pathToFile='./neurons.txt', ignoreCommentedLine=True):
  ...
  for line in split_lines[start:]:
    ...
    result = spine._getStatistics(type=type2Analyze)
    if (type2Analyze== 'all' or type=='surfaceArea'):
...
def getNeuronStatisticsFromSWC(pathToFile='./neurons/neurons.swc', ignoreCommentedLine=True):
  ...
  def getResult(result, neuronStat):
    ...
    if (type2Analyze== 'all' or type=='compartment'):
      ...
      if (neuronStat['compartment-surfacearea']):
        ...
        neuronStat['compartment-surfacearea-std'] = np.std(arr)
      if (neuronStat['compartment-volume']):
        ...
        neuronStat['compartment-volume-std'] = np.std(arr)
      if (neuronStat['compartment-length']):
        ...
        neuronStat['compartment-length-std'] = np.std(arr)
  ...
  if (0):
    ...
    result = spine._getStatistics(type=type2Analyze, branchType2Find = branchTypes)
...
class SomeClass(object):
    ...
    def parse_file(self):
        ...
        for line in split_lines[start:]:
            ...
            #    pdb.set_trace()
            parent_id = line[6]
            ###!if (int(parent_id) == -1) or (not self.children_lookup[parent_id]):
            if (not parent_id in self.children_lookup):
                self.children_lookup[parent_id] = [id]
            else:
                self.children_lookup[parent_id].append(id)
            if (int(line[6]) != -1):
                parent_id = line[6]
        ...
        #    if (int(parent_id) != -1):
        #        pdist2soma= self.point_lookup[parent_id]['dist2soma']
        #        print("DEBUG: ", dist2soma, pdist2soma)
        #quit()
        return
    ...
    def test(self):
        ...
        #     loc = self._nextTime(rateParameter)
    ...
    def checkSiteApical(self, startDist=615.0, endDist=625.0):
        ...
        for ix in range(len(self.point_lookup)):
            ...
            dis2points = self._find_distance(id, parent_id)
    ...
    def genMSN(self):
      ...
      #self.removeNearbyPoints(dist, write2File=True,fileSuffix='_revised.swc')
      #self.removeNearbyPointsByDist(dist, write2File=False)
      #self.regularizePointsByDist(dist, tolerance, write2File=False)
      self.regularizePointsByDist(dist, tolerance)
      #self.removeNearbyPointsByDist(dist, write2File=True,fileSuffix='_revised.swc')
      #volume = 3.5
      #self.removeNearbyPointsByVolume(volume, write2File=False)
      #####self.convertToTufted()
    ...
    def genSpine_PyramidalL5(self):
        ...
        while holdpoint_list:
            ...
            for id in holdpoint_list:
                ...
                #interval = self._nextLocation(mean_spine_distance)
                #print(interval)
                assert(interval > 0)
                ...
                if int(branchType) == self.__class__.branchType["basal"]:
                    ...
                    for i in range(numSpines):
                        ...
                        spine_distance2distalpoint =\
                            i * slot_len + \
                            spineLocationAsDistancetoDistalEndCurrentSlot
                        #gap = self._find_distance(distal_id, pid )
                        ...
                        dis2points = self._find_distance(distal_id, pid)
    ...
    def genSpine_MSN_distance_based(self, use_mean=True):
        ...
        ### USER-SELECTION
        ageType = 'young'  # 'young', 'aged', 'general'
        spineChoice = "generic" #{"generic": 0, "mush-thin":1}
        useMean = True # True, False  [where we generate random size for spines or not]
        ...
        #   spine="generic"= 1  ; bouton = "excitatory=2" or "inhibitory=3"
        #self.spineMType = {"generic": 2}
        #self.boutonMType = {"excitatory":4, "inhibitory":1}
        ...
        self.spineMType = {"generic": 2,
                           "thin": 2, "mush": 3}
        self.boutonMType= {"excitatory":4, "inhibitory":1,
                           "thin":4, "mush":5, "GABA": 1} #as a function of spine-type
        ###################
        ## SETTING 1
        #(no change)
        bouton = 'bouton_'
        ...
        #[pointID] = times
        timeVisitPoint = {}
        originalTotalPoints = len(self.point_lookup)
        #####Find soma
        for ix in range(len(self.point_lookup)):
            id = str(ix+1)
            parent_id = self.point_lookup[id]['parent']
        ...
        while holdpoint_list:
            ...
            for id in holdpoint_list:
                ...
                if (distance_slot[binslot_index] >
                    ...
                    #nearbyOriginalPoint[newid] =[]
                    if (pid < originalTotalPoints):
                        nearbyOriginalPoint[newid] = [int(pid)]
                    else:
                        nearbyOriginalPoint[newid] = [-1]
                        #nearbyOriginalPoint[newid].append(pid)
                    if (id < originalTotalPoints):
                        nearbyOriginalPoint[newid].append(int(id))
                    else:
                        nearbyOriginalPoint[newid].append(-1)
                else: # case 2 = face the branchpoint
                    length_region2consider = self.point_lookup[id]['dist2soma']\
                        - self.point_lookup[pid]['dist2soma']
                    if (self.point_lookup[pid]['dist2soma'] > 0):
                        dist2branchPoint=self.point_lookup[pid]['dist2branchPoint']
                        numChildren=self.point_lookup[pid]['numChildren']
                ...
                if int(branchType) == self.__class__.branchType["basal"]:
                    ...
                    for i in range(numSpines):
                        ...
                        parent_y1 = float(self.point_lookup[pid]['siteY'])
                        parent_z1 = float(self.point_lookup[pid]['siteZ'])
                        distance =  self._find_distance(distal_id, pid)
                        ...
                        siteY = np.interp(gap-spine_distance2distalpoint, [0, dis2points], [parent_y1, y1])
                        siteZ = np.interp(gap-spine_distance2distalpoint, [0, dis2points], [parent_z1, z1])
                        ### skip if it is too closed to tree-points
                        toleranceDistance = 0.5
                        if ((int(distal_id) <= originalTotalPoints) or
                                nearbyOriginalPoint[(distal_id)][0] != -1
                             ) and \
                                spine_distance2distalpoint < toleranceDistance:
                            continue
                        if (int(pid) <= originalTotalPoints or
                            nearbyOriginalPoint[(pid)][1] != -1
                            ) and \
                                gap - spine_distance2distalpoint < toleranceDistance:
                            continue
                        basalSpineCount  += 1
                        self.spineCount += 1
                        index = self.spineCount
                        ...
                        spineMType = self.spineMType["thin"]
                        if (spineChoice == "generic"):
                            boutonMType = self.boutonMType["excitatory"]
                            boutonFileName = "bouton_generic"
                            spineFileName = "spine_generic"
                            spineMType = self.spineMType["generic"]
                            #pass
                        elif (spineChoice == "mush-thin"):
                            randUniform = random.random()
                            if int(branchType) == self.branchType["basal"]:
                                if (randUniform <= proportionMushBasal):
                                    spineFileName = spineTypeMushBasal
    ...
    def genSpine_MSN_distance_based_test(self, use_mean=True):
        ...
        for idx, val in enumerate(true_spineoccurence_slot):
            ...
            spinesInfo = [self._nextLocation(lambda_slot[idx])
                          for i in
    ...
    def _genInhibitory(self, listBranches, mean_symsynapse_distance, stimPeriod,
        ...
        while holdpoint_list:
            ...
            for id in holdpoint_list:
                ...
                #interval = self._nextLocation(mean_symsynapse_distance)
                #print(interval)
                assert(interval > 0)
    ...
    def getSpineHeadFromTissueFile(self, x,y,z, r, ignoreCommentedLine=True):
      ...
      for line in split_lines[start:]:
        ...
        if (line[7] == 'R'):
          ...
          zoffset = float(line[6])
        spine  = SomeClass(file, False)
        [result, center] = spine._withSomaInside(x,y,z,r, xoffset, yoffset, zoffset)
        # if (spine._withSomaInside(x,y,z,r, xoffset, yoffset, zoffset)):
        if (result):
    ...
    def regularizePointsByDist(self, dist_criteria=7.0,
                               tolerance=1.0,
                               write2File=True,
                               fileSuffix='_regularizePointsByDist.swc'):
      """
      Readjust the data points so that the distance between them
      are as uniform as possible using 'dist_criteria'
      NOTE:
        we can revise this
      """
      threshold = dist_criteria * 0.2
      if (tolerance > threshold):
          print("ERROR: tolerance too big");
          print("... making smaller than ", threshold)
          return

      listExaminingPoints = ['-1']
      self.listExaminingPoints= list(set(listExaminingPoints))
      print("Add/remove points to ensure regular distance around (dist-criteria)------")
      print("Before regularize : ", len(self.point_lookup), " points")
      lineArray = []
      while (self.listExaminingPoints):
        ##
        #Step 2: delete these points and update others
        ##....
        index = 0
        pid = self.listExaminingPoints[index]
        if (int(pid) == -1):
            for lkindex, cid in enumerate(self.children_lookup[pid]):
                self.listExaminingPoints.append(cid)
                parent_id = self.point_lookup[cid]['parent']
                brType =self.point_lookup[cid]['type']
                x =float(self.point_lookup[cid]['siteX'])
                y =float(self.point_lookup[cid]['siteY'])
                z =float(self.point_lookup[cid]['siteZ'])
                r =float(self.point_lookup[cid]['siteR'])
                ##!add the point to Output list
                #lineArray.append([cid, str(brType),
                #                str(x), str(y),
                #                str(z), str(r), str(parent_id)])
            self.listExaminingPoints.pop(index)
            continue
        else:
            #!add the point to Output list
            parent_id = self.point_lookup[pid]['parent']
            brType =self.point_lookup[pid]['type']
            x =float(self.point_lookup[pid]['siteX'])
            y =float(self.point_lookup[pid]['siteY'])
            z =float(self.point_lookup[pid]['siteZ'])
            r =float(self.point_lookup[pid]['siteR'])
            lineArray.append([pid, str(brType),
                              "{0:.3f}".format(x),
                              "{0:.3f}".format(y),
                              "{0:.3f}".format(z),
                              "{0:.3f}".format(r),
        ...
        for lkindex, cid in enumerate(self.children_lookup[pid]):
            ...
            y1 = float(self.point_lookup[cid]['siteY'])
            z1 = float(self.point_lookup[cid]['siteZ'])
            branchOrder = self.point_lookup[cid]["branchOrder"]
            parent_x1 = float(self.point_lookup[pid]['siteX'])
            parent_y1 = float(self.point_lookup[pid]['siteY'])
            parent_z1 = float(self.point_lookup[pid]['siteZ'])
            ...
            ##print("dist: ", distance)
            jumpedIn=-1
            if (distance >= 2 * dist_criteria):
                jumpedIn=1
                #!create new point with normal distance
                interval = dist_criteria
                X = np.interp(distance-interval, [0, distance], [parent_x1, x1])
                Y = np.interp(distance-interval, [0, distance], [parent_y1, y1])
                Z = np.interp(distance-interval, [0, distance], [parent_z1, z1])
                Rp = float(self.point_lookup[pid]['siteR'])
                Rc = float(self.point_lookup[cid]['siteR'])
                Ra = Rp +  interval / distance * (Rc - Rp)
                ...
                self.point_lookup[newid] = {'type': self.point_lookup[cid]['type'],
                                        ...
                                        'numChildren': 1}
                self.point_lookup[cid]['parent'] = str(newid)
                ##print("before ", pid, "whild: ", self.children_lookup[pid])
                self.children_lookup[pid][lkindex] = newid
                ##print("    ", self.children_lookup[pid])
                self.children_lookup[newid] = [cid]
                self.listExaminingPoints.append(newid)
                #pass
            elif (distance >= 2 * dist_criteria - threshold):
                jumpedIn=2
                #!create new point with adjusted distance
                interval = distance/2.0
                X = np.interp(distance-interval, [0, distance], [parent_x1, x1])
                Y = np.interp(distance-interval, [0, distance], [parent_y1, y1])
                Z = np.interp(distance-interval, [0, distance], [parent_z1, z1])
                Rp = float(self.point_lookup[pid]['siteR'])
                ...
                if (0):
                    ...
                    ##or this
                    #!add the point to Output list
                    brType =self.point_lookup[newid]['type']
                    x =float(self.point_lookup[newid]['siteX'])
                    y =float(self.point_lookup[newid]['siteY'])
                    z =float(self.point_lookup[newid]['siteZ'])
                    r =float(self.point_lookup[newid]['siteR'])
                    parent_id = self.point_lookup[newid]['parent']
                    lineArray.append([newid, str(brType),
                                      "{0:.3f}".format(x),
                                      "{0:.3f}".format(y),
                                      "{0:.3f}".format(z),
                                      "{0:.3f}".format(r),
                                      str(parent_id)])
                    self.listExaminingPoints.append(cid)
            #elif (distance k= dist_criteria - threshold):
            #    #!no point created
            #    self.listExaminingPoints.append(cid)
            #elif (distance >= dist_criteria - threshold):
            ##NOTE: may consider if the point is the tip-point remove it if it's short
            else:
                jumpedIn=3
                #!no point created
                numChildren = int(self.point_lookup[cid]['numChildren'])
                if (numChildren > 0):
                    self.listExaminingPoints.append(cid)
                else:
                    #!terminal point
        ...
        self.listExaminingPoints.pop(index)
        #nothing afer this (when the system is completed)
      #pass

      print("After regularize : ", len(self.point_lookup), " points")
      lineArray.sort(key=lambda x: int(x[0]))
      ##lineArray.sort(key=lambda x: int(x[6]))
      lineArray = np.asarray(lineArray)
      if (write2File):
        PL5bFileName = self.swc_filename+fileSuffix
        np.savetxt(PL5bFileName, lineArray, fmt='%s')
    ...
    def addNewPoints(self, parentLineIndex, write2File=True,
                           fileSuffix='_addedPoints.swc'):
      """
      Add one or many points based on the given direct parent line index
      """
      ##
      #Step 1: delete these points and update others
      ##....
      linesParent = []
      linesParent.extend(parentLineIndex)
      lineArray = []
      index = 0
      mapNewId = {}
      tmpPointLookup = deepcopy(self.point_lookup)
      print("Before add: ", len(self.point_lookup), " points")
      for ix in range(len(self.point_lookup)):
          id = str(ix+1)
          parent_id = self.point_lookup[id]['parent']
          brType =self.point_lookup[id]['type']
    ...
    def getSpineHeadFromFolder(self, x,y,z, r):
      ...
      for file in glob.glob(pathTofiles):
        spine  = SomeClass(file, False)
        if (spine._withSomaInside(x,y,z,r)):
          print (file)
    ...
    def getPreSynapticSomaHeadFromTissueFile(self, x,y,z, r, ignoreCommentedLine=True):
      ...
      for line in split_lines[start:]:
        ...
        spine  = SomeClass(file, False)
        # if (spine._withSomaInside(x,y,z,r, xoffset, yoffset, zoffset)):
        if (spine._withAxonInside(x,y,z,r, xoffset, yoffset, zoffset)):
          print (file, ", boutonMType: ", boutonMType)

    def _getStatistics(self, type='all', branchType2Find=[
                                                    branchType['soma'],
                                                    branchType['axon'],
                                                    branchType['basal'],
                                                    branchType['apical'],
        ...
        type = 'all', 'surfaceArea', 'volume', 'radius', 'compartment'
        ...
        #time.sleep(10)
        #sys.exit
        print "Count : ", result['compartment-count']
        return result

    def _withSomaInside(self, x,y,z,r, xoffset=0.0, yoffset=0.0, zoffset=0.0):
      """
    ...
    def _withAxonInside(self, x,y,z,r, xoffset=0.0, yoffset=0.0, zoffset=0.0):
      """
      Return TRUE if the soma falls within the given sphere defined by
      (x,y,z,radius)
      """
                ...
                if (parent_id == idparent):
                    ...
                    if (self.point_lookup[id]['numChildren'] == 1):
                        self.point_lookup[id]['dist2branchPoint'] = \
                            self._find_distance(id, parent_id) + \
                            parent_dist2branchPoint
    ...
    def _find_distance(self, id1, id2):
        """
    ...
    def _nextTime(self, rateParameter):
        """
    ...
    def _nextLocation(self, rateParameter):
        """
