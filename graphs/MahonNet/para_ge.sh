...
variables=`echo $(seq 0 0.005 0.03)`
#variables=(0.033) #,0.041)

cb=0.2; #$(expr "scale = 5; 0.15" | bc | sed 's/^\./0./' );
#var1=0.0045;
synb=0.095;
deltat=0.2;
size=50; # size*size cell network 
tscale=1.0;
noiselev=0.0;
...
run_serial () {
    ...
    local ge=$2
    #[the same] for var1 in ${variables[@]}; do
    for var1 in $variables; do
      #local FOLDERSUFFIX="ge"$ge"_gi"$var

      local GSL_FILE=${changeFile}_$FOLDERSUFFIX.run
      local DATA_FOLDER="output_new_$FOLDERSUFFIX"
