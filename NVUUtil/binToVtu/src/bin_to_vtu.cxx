...
#define TIME_INT 0 
#define TIME_FLOAT 1 
//#define USE_TIME TIME_INT   // UC code
#define USE_TIME TIME_FLOAT // IBM code
...
int main(int argc, char *argv[])
{
  ...
  /*NOTE
   * The length L0_VALUE is 200 micrometer or 200*1e-6 or 2e-4 meter
   * So, BLOCK_LENGTH must be 2x this L0_VALUE
   */
// General parameters:
#define BLOCK_LENGTH \
  4e-4  // Length of one tissue block so it matches with output size of tree
        // (don't change).
  char Prefix[] = "";
...
// run with no arguments for debugging
  ...
  int conf_array[8];  // Create temporary array that stores parameters from
                      // configuration file (info.dat).
  ...
  {
    std::cerr<< " tree->x " << xCoord[_i] << "; tree->y " << 
      yCoord[_i] <<
      std::endl;
  }
#endif
  ...
  {
    std::cout << "Block: " << i << "\t x-coord: " << xCoord[i]
              << "  \t y-coord: " << yCoord[i]
              << std::endl;  // Print all coordinates.
  }
  ...
  double points_xorigin =
      xCoord[0] - (BLOCK_LENGTH / 2);  // Origin - x coordinate.
  double points_yorigin =
      yCoord[0] - (BLOCK_LENGTH / 2);  // Origin - y coordinate.
  ...
  for (int j = 0; j <= n_cols; j++)
  {
    ...
    for (int i = 0; i <= n_rows; i++)
    {
      ...
      points_tb->InsertNextPoint(points_xcoord, points_ycoord,
                                 0);  // x,y,z coordinates
    }
  }
  ...
  int n_nodes = POW_OF_2(n_levels - 1) -
                1;  // Number of nodes. (?) - Leaf nodes excluded!

  double xpoints_tree[n_branches];  // x coordinates for points for tree -
                                    // branches & pressure.
  double ypoints_tree[n_branches];  // y coordinates for points for tree -
                                    // branches & pressure.
...
#pragma omp parallel for
  ...
  {
    xpoints_tree[i] =
        (xpoints_tree[nz1[i - n_blocks]] + xpoints_tree[nz2[i - n_blocks]]) / 2;
    ypoints_tree[i] =
        (ypoints_tree[nz1[i - n_blocks]] + ypoints_tree[nz2[i - n_blocks]]) / 2;
    points_tree->InsertNextPoint(
        (xpoints_tree[nz1[i - n_blocks]] + xpoints_tree[nz2[i - n_blocks]]) / 2,
        (ypoints_tree[nz1[i - n_blocks]] + ypoints_tree[nz2[i - n_blocks]]) / 2,
        BLOCK_LENGTH);
  }
  ...
  // 2. Create cell array:
  vtkSmartPointer<vtkCellArray> cellArray_tb =
      vtkSmartPointer<vtkCellArray>::New();  // Create a cell array to store
                                             // hexahedrons in.
  vtkSmartPointer<vtkHexahedron> hexahedron =
      vtkSmartPointer<vtkHexahedron>::New();  // Create hexahedrons (3D blocks)
  vtkSmartPointer<vtkLine> lines = vtkSmartPointer<vtkLine>::New();
  vtkSmartPointer<vtkCellArray> cellArray_tree =
      vtkSmartPointer<vtkCellArray>::New();
  // 2.1 and 2.1 are completely independent. Process them using multiple threads

  // 2.1 Tissue blocks:
  double npoints_offset =
      (n_rows + 1) * (n_cols + 1);  // number of points - offset for 2nd "layer"
  ...
  for (int block_id = 0; block_id < n_blocks; block_id++)
  {
    ...
    hexahedron->GetPointIds()->SetId(
        4, i + npoints_offset);  // upper SW corner of hexahedron
    hexahedron->GetPointIds()->SetId(
        5, j + npoints_offset);  // upper SE corner of hexahedron
    hexahedron->GetPointIds()->SetId(
        6, k + npoints_offset);  // upper NE corner of hexahedron
    hexahedron->GetPointIds()->SetId(
        7, l + npoints_offset);  // upper NW corner of hexahedron

    cellArray_tb->InsertNextCell(hexahedron);  // Add hexahedrons to cellArray
  }

  // 2.2 H-Tree:
  for (int line_id = 0; line_id < n_branches - 1;
       line_id++)  // -1, because of root branch
  {
  ...
  cellArray_tree->InsertNextCell(
      lines);  // Create a cell array to store hexahedrons in and add them to it
...
#elif USE_TIME == TIME_FLOAT
  int num_time_steps = conf_array[7];
#endif
...
#pragma omp parallel
  {
    ...
    if (thread_id == 0)
    {
      std::cout << "Using " << num_threads
                << " OpenMP threads to process all time steps." << std::endl;
    }
  }
#endif
  ...
  for (int i = 0; i <= num_time_steps;)
  {
    ...
    if (i == 0)
      ...
      ts_in_buffer = ((num_threads + i) <= num_time_steps)
                         ? num_threads
                         : (num_time_steps - i + 1);

    double *tissue_buffer =
        (double *)malloc(tissue_seg_size * ts_in_buffer * sizeof(double));
    double *flow_buffer =
        (double *)malloc(flow_seg_size * ts_in_buffer * sizeof(double));

    if (tissue_buffer == NULL or flow_buffer == NULL)
    {
      std::cout
          << "Unable to allocate memory to read tissue or flow binary data"
          << std::endl;
      return EXIT_FAILURE;
    }

    is_tissue.read((char *)tissue_buffer,
...
#pragma omp parallel for firstprivate(i)
    for (int t = 0; t < ts_in_buffer; t++)
    {
      ...
      vtkSmartPointer<vtkUnstructuredGrid> uGrid2 =
          vtkSmartPointer<vtkUnstructuredGrid>::New();
#pragma omp critical
      {
        ...
        uGrid2->SetCells(VTK_LINE,
                         cellArray_tree);  // uGrid2->SetLines(cellArray2);
      }
...
#pragma omp critical
      ...
      // 4.1.1 Tissue blocks:
      std::vector<vtkSmartPointer<vtkDoubleArray> > stateVars;  // Create vector
                                                                // of arrays for
                                                                // each state
                                                                // variable.
      ...
      {
        vtkSmartPointer<vtkDoubleArray> array_tb =
            vtkSmartPointer<vtkDoubleArray>::New();  // Create array.
      ...
      vtkSmartPointer<vtkDoubleArray> array_tree =
          vtkSmartPointer<vtkDoubleArray>::New();  // Create array.
      ...
      vtkSmartPointer<vtkDoubleArray> array_tree_unscaled =
          vtkSmartPointer<vtkDoubleArray>::New();  // Create array.
      array_tree_unscaled->SetName(
          "blood_flow_unscaled");  // Only one array for flow variables.
      ...
      for (int k = 0; k < n_blocks; k++)
      {
        ...
        temp_array_tb[0] =
            20 * temp_array_tb[0];  // Convert radius to um from nondimensional
        temp_array_tb[9] = 0.001 * temp_array_tb[9];    // Convert Kp to mM
        temp_array_tb[24] = 0.001 * temp_array_tb[24];  // Convert Ke to mM

        temp_array_tb[2] =
            temp_array_tb[2] / temp_array_tb[1];  // Convert N_Na_k to Na_k;
        temp_array_tb[3] =
            temp_array_tb[3] / temp_array_tb[1];  // Convert N_k_k to K_k;
        temp_array_tb[4] =
            temp_array_tb[4] / temp_array_tb[1];  // Convert N_HCO3_k to HCO3_k;
        temp_array_tb[5] =
            temp_array_tb[5] / temp_array_tb[1];  // Convert N_Cl_k to Cl_k;

        temp_array_tb[6] =
            temp_array_tb[6] /
            (8.79e-8 - temp_array_tb[1]);  // Convert N_Na_s to Na_s;
        temp_array_tb[7] =
            0.001 * temp_array_tb[7] /
            (8.79e-8 - temp_array_tb[1]);  // Convert N_K_s to K_s in mM;
        temp_array_tb[8] =
            temp_array_tb[8] /
            (8.79e-8 - temp_array_tb[1]);  // Convert N_HCO3_s to HCO3_s;
...
#pragma omp critical
      ...
      for (int level = 0; level < n_levels; level++)
      {
        ...
        {
          flowVar[0]->InsertNextValue(temp_array_tree[i] *
                                      pow(2, ((n_levels - level - 1))));
...
#pragma omp critical
        ...
        vtkSmartPointer<vtkDoubleArray> flowArray =
        ...
        tubeFilter->SetRadius(
            0.00014 * (0.05 * stateVars[0]->GetValue(line_id)) -
            0.0001);  // varying radii - different depending on NVU version
                      // TODO: make this dependent on max and min values of
                      // radius.
      ...
      for (int line_id = n_blocks; line_id < n_branches - 1;
           line_id++)  // -1, because of root branch
      {
        vtkSmartPointer<vtkLineSource> lineSource =
            vtkSmartPointer<vtkLineSource>::New();
...
#pragma omp critical
        ...
        tubeFilter->SetInputData(branchPolyData);
        tubeFilter->SetRadius(5e-5);  // TODO: each level include different
                                      // radius! (5e-5*pow(2,0.5*level))
...
#pragma omp critical
      ...
      vtkSmartPointer<vtkXMLUnstructuredGridWriter> writer_tb =
          vtkSmartPointer<vtkXMLUnstructuredGridWriter>::New();  // save uGrid
                                                                 // to file
      ...
      vtkSmartPointer<vtkXMLPolyDataWriter> writer_tree =
      ...
      vtkSmartPointer<vtkXMLUnstructuredGridWriter> writer2 =
          vtkSmartPointer<vtkXMLUnstructuredGridWriter>::New();  // save uGrid2
                                                                 // to file
