...
PrintFunctorType::PrintFunctorType() {}

void PrintFunctorType::getFunctor(std::unique_ptr<Functor> & r_aptr)
{
   r_aptr.reset(new PrintFunctor());
}


std::string PrintFunctorType::getName()
{
   return std::string("Print");
}


std::string PrintFunctorType::getDescription()
{
   return std::string(Functor::_category);
}


void PrintFunctorType::getQueriable(std::unique_ptr<InstanceFactoryQueriable>& dup)
{
...
Functor* PrintFunctorType::getFunctor()
{
   Functor* f = new PrintFunctor;
