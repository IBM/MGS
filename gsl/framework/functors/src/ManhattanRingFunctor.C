...
void ManhattanRingFunctor::doInitialize(LensContext *c, 
					const std::vector<DataItem*>& args)
{
   ...
   */
   ...
   // prototype SamplingFctr2 ManhattanRing(std::list<int>,int direction);

   IntArrayDataItem *ia_di = dynamic_cast<IntArrayDataItem*>(args[0]);
   if (ia_di==0) {
      throw SyntaxErrorException(
	 "ManhattanRingFunctor: expected list of ints in initialization");
   }
   _list = *ia_di->getIntVector();
}


void ManhattanRingFunctor::doExecute(LensContext *c, 
				     const std::vector<DataItem*>& args, 
				     std::auto_ptr<DataItem>& rvalue)
{
   ...
   switch(resp) {
      ...
      case ConnectionContext::_BOTH:
         throw SyntaxErrorException(
	    "ManhattanRingFunctor: invalid responsibility specification");
   }
   ...
   if (cc->restart) {
	 ...
	 msg << "ManhattanRing: mismatch in size of radius list and sample set: "
   ...
   if (current.size() == 0) {
      throw SyntaxErrorException(
	 "ManhattanRing: No nodes available at requested radius");
   }
...
void ManhattanRingFunctor::collectRadialNodes(
   ...
   std::vector<std::vector<NodeDescriptor*> >& collectedRadialNodes)
{
   ...
   if (origin.size() != dims) {
      throw SyntaxErrorException(
	 "ManhattanRing: the origin coordinates are incompatible with the NodeSet dimensions");
   }
