...
void SumFunctor::doExecute(LensContext *c, 
			   const std::vector<DataItem*>& args, 
			   std::auto_ptr<DataItem>& rvalue)
{
   ...
   if ( idi ) {
	 ...
	 begin = ddi->getModifiableDataItemVector()->begin(),
      ...
      for ( iter = begin; iter != end; ++iter )
         _di_array->push_back(*iter);
      _float_array = 0;
      _int_array = 0;
   }
   else {
      throw SyntaxErrorException(
	 "Dynamic cast of DataItem to Int or Float ArrayDataItem failed in SumFunctor");
   }
   ...
   if ( _int_array ) {
	 ...
	 begin = _int_array->begin(), 
      ...
      rvalue.reset(i_di);
      std::cout << "Executed SumFunctor, returning int "
		<< i_di->getInt() << "." << std::endl;
   }
   else if ( _float_array ) {
      float _f_total = 0.0;
      std::vector<float>::iterator iter, 
	 begin = _float_array->begin(), end = _float_array->end();
      for ( iter = begin; iter != end; ++iter )
         _f_total += *iter;
      FloatDataItem *f_di = new FloatDataItem;
      f_di->setFloat(_f_total);
      rvalue.reset(f_di);
      std::cout << "Executed SumFunctor, returning float " 
		<< f_di->getFloat() << "." << std::endl;
   }
   else if ( _di_array ) {
      float total = 0.0;
      std::vector<DataItem*>::iterator iter, 
	 begin = _di_array->begin(), end = _di_array->end();
      for ( iter = begin; iter != end; ++iter ) {
         if ( strcmp ( (*iter)->getType(), "INT" ) == 0 )
            total += (float) dynamic_cast<IntDataItem*>(*iter)->getInt();
         else if( strcmp ( (*iter)->getType(),"FLOAT" ) == 0 ) {
