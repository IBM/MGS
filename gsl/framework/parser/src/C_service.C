...
void C_service::internalExecute(LensContext *c)
{
   if (_queryPathProduct) {
      ...
      DataItem* di = const_cast<DataItem*>(
	 ...
	 if (esdi) {
	    ...
	    if (vdi) {
//	       _service = vdi->getVariable()->getPublisher()->getService(
	       _service = vdi->getVariable()->getVariable()->getPublisher()->getService(   // added by Jizhu Lu on 03/16/2006
		  *_string);
	    } else {
	       assert(0);
	    }
...
C_service::C_service(C_query_path_product* qpp, SyntaxError* error)
   : C_production(error), _queryPathProduct(qpp), _declarator(0), _string(0), 
     _service(0)
{
}

C_service::C_service (C_declarator* ad, std::string* as, SyntaxError* error)
   : C_production(error), _queryPathProduct(0), _declarator(ad), _string(as), 
     _service(0)
{
}
...
C_service::C_service(const C_service& rv)
   : C_production(rv), _queryPathProduct(0), _declarator(0), _string(0),
...
C_service* C_service::duplicate() const
{
   return new C_service(*this);
}
