...
GslContext::GslContext(Simulation* s)
...
void GslContext::addCurrentRepertoire(Repertoire* rep)
{
...
GslContext::GslContext(const GslContext& lc)
...
void GslContext::addStatement(C_production* statement)
{
   _statements.push_back(statement);
}

void GslContext::execute() 
{
...
void GslContext::getCurrentPhase(std::unique_ptr<Phase>& phase) const
{
   _currentPhase->duplicate(phase);
}

void GslContext::setCurrentPhase(std::unique_ptr<Phase>& phase)
{
   delete _currentPhase;
   _currentPhase = phase.release();
}

void GslContext::duplicate(std::unique_ptr<GslContext>& dup) const
{
   dup.reset(new GslContext(*this));
}
