...
void C_grid_function_name::internalExecute(LensContext *c, Grid* grid)
{
   ...
   _argList->execute(c);
   if (_type == _LAYER) layers(c,grid);
   else if (_type == _INITNODES) initNodes(c,grid);
}
...
C_grid_function_name::C_grid_function_name(C_argument_list *a, 
					   SyntaxError * error)
   : C_production_grid(error), _type(_INITNODES), _argList(a), _declarator(0)
{
}
...
void C_grid_function_name::initNodes(LensContext *c, Grid * grid)
{
   ...
   const NodeSetDataItem* nsdi = 
      dynamic_cast<const NodeSetDataItem*>(dataItem);
   if (nsdi == 0) {
   ...
   for (unsigned it = 1; it < args->size(); ++it) {
      // now get the NodeInit functors
      dataItem = *a_iter++;
      const FunctorDataItem* fdi = 
	 dynamic_cast<const FunctorDataItem*>(dataItem);
      if (fdi == 0) {
	 std::string mes = "Subsequent argument of 'initnodes' is not a functor.";
	 throwError(mes);
      }
      Functor* functor = fdi->getFunctor();
      // comparing char*'s are wrong, fix Functor base class.
      std::string category=functor->getCategory();
      if (strcmp(functor->getCategory(),NodeInitializerFunctor::_category) != 0) {
	 std::string mes = 
	    "Subsequent argument of 'initnodes' is not a NodeInitializer functor, instead it is: ";
...
void C_grid_function_name::layers(LensContext *c, Grid *grid)
{
   ...
   Functor* functor = fdi->getFunctor();
   std::string category=functor->getCategory();
   if (strcmp(functor->getCategory(), LayoutFunctor::_category) != 0) {
      std::string mes = "Third argument of 'layer' is not a Layout functor, instead it is: ";
      mes += functor->getCategory();
      throwError(mes);
   }
   ...
   if (args->size() == 4) {
     ...
     if (c->sim->isGranuleMapperPass() || c->sim->isCostAggregationPass()) {
       ...
       coord[0] = 0;
       std::vector<int>::const_reverse_iterator iter2 = sz.rbegin(), end2 = sz.rend();
	 ...
	 coord[0]++;
       ...
       coord[0] = 0;
       std::vector<int>::const_iterator iter = density->begin(), end = density->end();
       for (; iter != end; ++iter) {
   ...
   // add the layer
   try {
      grid->addLayer(*density, name, model, *ndpairlist, granuleMapper->getIndex());
