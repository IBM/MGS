...
inline void SBrowser::sendDescriptor(QueryDescriptor& qd)
{
   ...
   for(unsigned i=0; i < v.size(); i++) {
      ...
      if(v[i]->getType() == QueryField::ENUM) {
         ...
         _UI->getServer()->sendToClient(
	    "display browser queryfield enums begin");
         ...
         _UI->getServer()->sendToClient(
	    "display browser queryfield enums end");
      }
      else {
         char buff2[MAXBUFFSIZE];
...
inline void SBrowser :: cmdHandler(const char* cmd)
{
   ...
   if(strncmp(cmd, "browse query queryfields", 24)==0) {
   ...
   if(strncmp(cmd, "browse get service dataitem", 27)==0) {
	 ...
	 Service* s = p->getService(serviceName);
         ...
         os 
	    <<"display extended_error <>The server returned the following: <> Could"
	    <<" not complete requested action. The item you have selected does not "
	    <<"have a Publisher, so no service can be retrieved from it. There may "
	    <<"be a mismatch in what you have selected, and what the server thinks "
	    <<"you have selected.\n Item selected: \""<<n<<"\".<>";
   ...
   if(strncmp(cmd, "browse view service",19)==0) {
      if(_currentQ->isPublisherQueriable()) {
	    ...
	    << std::endl
	    << "\t" << _currentQ->getQueriableDescriptor().getName().c_str()
	    << std::endl 
	    << "\t" << _currentQ->
	    getQueriableDescriptor().getDescription().c_str()
	    << std::endl;
      }
   }
   ...
   if(strcmp(cmd, "browse back")==0) {
      std::cout << "SBrowser: browser history size =" 
		<< _historyQ.size() <<  std::endl;
   ...
   if(strncmp(cmd, "browse select queriable", 23)==0) {
      ...
      if( (selection >=0) && (selection < qlist.size())) {
         ...
         if(tdi) {
            std::cout << "SBrowser: got a TriggerDataItem, sending to client to display." << std::endl;
            std::ostringstream os;
...
inline int SBrowser:: parseQuery(const char* q)
{
   ...
   while( (label!= NULL) && (i <= qfs.size()) ) {
      ...
      os <<"The sytem could not perform query. There was a mismatch when trying to fill";
      ...
      if(strcmp(label, "enum")==0) {
         ...
         if( qfs[i]->getType()!=QueryField::ENUM ) {
            std::cout <<"SBrowser: parseQuery: Trying to select enum from something that isn't an ENUM queryfield..., it is of type ";
            std::cout << qfs[i]->getType()<< std::endl;
            os <<" Tried to select an enumeration from a VALUE queryfield.\n ";

            os <<" The Queriable in question was: ";
            os << _currentQ->getQueriableDescriptor().getName()<<"<>"<<std::endl;
            os <<"It is possible that you did not fill all the queryfileds properly. \n";
            char errbuff[MAXBUFFSIZE];
            strcpy(errbuff, os.str().c_str());
            _UI->getServer()->sendToClient(errbuff);
         }
         ...
         if(qfs[i]->getType()!=QueryField::VALUE ) {
            std::cout <<"SBrowser: parseQuery: This is not a VALUE queryfield, its a ";
            std::cout << qfs[i]->getType() << std::endl;
            ...
            os <<" The Queriable in question was: ";
            os << _currentQ->getQueriableDescriptor().getName()<<"<>"<<std::endl;
            os <<"It is possible that you did not fill all the queryfields properly.\n";
            char errbuff[MAXBUFFSIZE];
            strcpy(errbuff, os.str().c_str());
            _UI->getServer()->sendToClient(errbuff);
         }
...
inline std::string SBrowser::getCommand()
{
   ...
   if ((cmd == "control start")||(cmd == "control resume") || (cmd == "control stop")) {
      _i = 0;
   }
   return cmd;
}
...
inline int SBrowser::getSelection(const std::string & cmd)
{
   ...
   if( (strcmp(s,"control stop") ==0) || (strcmp(s, "control resume")==0) ) {
      return 0;
   }
...
inline bool SBrowser::isMatch(std::string input, std::string comp)
{
   ...
   if (input == comp) rval = true;
   ...
   else if (((*inp_iter)+32 == (*cmp_iter)) 
	    && (input.length()==1)) rval = true;
   else {
      rval = true;
      if (input.length() == comp.length()) {
         ...
         for (cmp_iter = comp.begin(); cmp_iter != cmp_end; cmp_iter++) {
            if (((*inp_iter) != (*cmp_iter)) 
		&& ((*inp_iter)+32 != (*cmp_iter))) rval = false;
            inp_iter++;
         }
      }
      else rval = false;
