...
BoolTrigger::BoolTrigger(Simulation& sim, std::vector<DataItem*> const& args)
  ...
  if (ptrSvcDI == 0) {
    std::cerr
        << "Dynamic cast of DataItem to ServiceDataItem failed on BoolTrigger!"
        << std::endl;
    exit(-1);
  }
  ++iter;
  ...
  if (criterionDI == 0) {
    std::cerr
        << "Dynamic cast of DataItem to NumericDataItem failed on BoolTrigger!"
        << std::endl;
    exit(-1);
  }
  ...
  if (phaseDI == 0) {
    std::cerr
        << "Dynamic cast of DataItem to PhaseDataItem failed on BoolTrigger!"
        << std::endl;
    exit(-1);
  }
...
void BoolTrigger::setEvaluator(std::string op) {
  if (_op == "==")
    _evaluator = &BoolTrigger::isEqual;
  else if (_op == "!=")
    _evaluator = &BoolTrigger::isNotEqual;
  else {
...
bool BoolTrigger::isEqual() { return (*_service) == _criterion; }
