...
UnsignedTrigger::UnsignedTrigger(Simulation& sim, std::vector<DataItem*> const & args)
   : TriggerBase(), _sim(sim)
{
   ...
   if (descriptionDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to StringDataItem failed on UnsignedTrigger!"<<std::endl;
      exit(-1);
   }
   ...
   if (ptrSvcDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to ServiceDataItem failed on UnsignedTrigger!"<<std::endl;
      exit(-1);
   }
   ++iter;
   ...
   if (criterionDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to NumericDataItem failed on UnsignedTrigger!"<<std::endl;
      exit(-1);
   ...
   if (phaseDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to PhaseDataItem failed on UnsignedTrigger!"<<std::endl;
      exit(-1);
   }
...
{
   if (_op == ">") _evaluator = &UnsignedTrigger::isGreaterThan;
   else if (_op == "<") _evaluator = &UnsignedTrigger::isLessThan;
   else if (_op == "==") _evaluator = &UnsignedTrigger::isEqual;
   else if (_op == "!=") _evaluator = &UnsignedTrigger::isNotEqual;
   else if ((_op == ">=") || (_op == "=>")) _evaluator = &UnsignedTrigger::isGreaterThanOrEqual;
   else if ((_op == "<=") || (_op == "=<")) _evaluator = &UnsignedTrigger::isLessThanOrEqual;
   else if (_op == "!%") _evaluator = &UnsignedTrigger::isModulusZero;
   else if (_op == "%") _evaluator = &UnsignedTrigger::isModulusNonZero;
   else {
...
bool UnsignedTrigger::isEqual()
{
   return (*_service) == _criterion;
}


bool UnsignedTrigger::isGreaterThan()
{
   return (*_service) > _criterion;
}


bool UnsignedTrigger::isLessThan()
{
   return (*_service) < _criterion;
}


bool UnsignedTrigger::isNotEqual()
{
   return (*_service) != _criterion;
}
...
bool UnsignedTrigger::isLessThanOrEqual()
{
   return (*_service) <= _criterion;
}


bool UnsignedTrigger::isModulusZero()
{
   return !((*_service) % _criterion);
}


bool UnsignedTrigger::isModulusNonZero()
{
   return (*_service) % _criterion;
}
