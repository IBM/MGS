...
FloatTriggerDescriptor::FloatTriggerDescriptor(Simulation& s)
   : TriggerType("FloatTrigger", "FloatTrigger", "Can trigger on any float."), 
     _sim(s)
{
...
void FloatTriggerDescriptor::getQueriable(
...
Trigger* FloatTriggerDescriptor::getTrigger(std::vector<DataItem*> const & args)
{
   Trigger* rval = new FloatTrigger(_sim, args);
...
Trigger* FloatTriggerDescriptor::getTrigger(NDPairList& ndp)
{
   Trigger* rval = 0;
   ...
   if(itr2 == end) {
      throw SyntaxErrorException("Service not found in FloatTriggerDescriptor NDPList!");
   }
   else if(itr3 == end) {
      ...
      throw SyntaxErrorException("Delay not found in FloatTriggerDescriptor NDPList!");
   }
   else {
	 ...
	 args.push_back((*itr)->getDataItem());
      ...
      rval = new FloatTrigger(_sim, args);
      _triggerList.push_back(rval);
      TriggerDataItem* tdi = new TriggerDataItem;
      tdi->setTrigger(rval);
      _instances.push_back(tdi);
...
void FloatTriggerDescriptor::duplicate(
   std::auto_ptr<TriggerType>& dup) const
{
   dup.reset(new FloatTriggerDescriptor(*this));
}

FloatTriggerDescriptor:: ~FloatTriggerDescriptor()
{
}
