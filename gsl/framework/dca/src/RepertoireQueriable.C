...
RepertoireQueriable::RepertoireQueriable(Repertoire* repertoire)
{
   ...
   if (gridRep) {
      ...
      std::unique_ptr<QueryField> aptr_gridQF(new QueryField(QueryField::ENUM));
      ...
      std::unique_ptr<EnumEntry> aptrEnumEntry(new EnumEntry("Grid", "Provide queriable Grid."));
      ...
      std::unique_ptr<QueryField> aptr_subRepertoireQF(new QueryField(QueryField::ENUM));
      ...
      for (std::list<Repertoire*>::const_iterator iter = _subRepertoires.begin(); iter != end; ++iter) {
         ...
         std::unique_ptr<EnumEntry> aptrEnumEntry(new EnumEntry((*iter)->getName(), descr));
   ...
   if (_connectionSetMap.size() > 0) {
      std::unique_ptr<QueryField> aptr_connectionSetPreQF(new QueryField(QueryField::ENUM));
      ...
      std::unique_ptr<QueryField>  aptr_connectionSetPostQF(new QueryField(QueryField::ENUM));
      ...
      map_iter != map_end; ++map_iter) {
         ...
         std::unique_ptr<EnumEntry> aptrEnumEntry(new EnumEntry(prePtr->getName(), prePtr->getModelName()));
         ...
         for (std::list<ConnectionSet*>::iterator list_iter = cnxnSetList.begin(); list_iter != list_end; list_iter++) {
            ...
            if (!post_enum_found) {
               std::unique_ptr<EnumEntry> aptrEnumEntry(new EnumEntry(postPtr->getName(), postPtr->getModelName()));
...
void RepertoireQueriable::getDataItem(std::unique_ptr<DataItem> & apdi)
{
...
std::unique_ptr<QueryResult> RepertoireQueriable::query(int maxItem, int minItem, int searchSize)
{
   ...
   if (_queryDescriptor.getQueryFields().size()) {
      ...
      if (_repertoire->isGridRepertoire()) {
         if (queryFields[_gridIdx]->getField() == "Grid") {
            qr->_numFound++;
            std::unique_ptr<Queriable> aptrQueriable(new GridQueriable(_repertoire->getGrid()));
            ...
            for (std::list<Repertoire*>::const_iterator iter = _subRepertoires.begin(); iter != end; iter++) {
               if (queryFields[_subRepertoireIdx]->getField() == (*iter)->getName()) {
                  ...
                  if ((qr->_numFound>maxItem) || (qr->_numFound-minItem>searchSize)) {
                     ...
                     std::unique_ptr<Queriable> aptrQueriable(new RepertoireQueriable(*iter));
                     qr->addQueriable(aptrQueriable);
                  }
               }
            }
   ...
   for (std::map<GridLayerDescriptor*, std::list<ConnectionSet*> >::const_iterator iter = _connectionSetMap.begin(); iter != map_end; iter++) {
      if (preName == (*iter).first->getName()) {
         ...
         for (std::list<ConnectionSet*>::iterator iter2 = cnxnSetList.begin(); iter2 != list_end; iter2++) {
            ...
            if (postName == "") {
               std::unique_ptr<Queriable> aptrQueriable(new ConnectionSetQueriable(*iter2));
               qr->addQueriable(aptrQueriable);
            }
            else {
...
void RepertoireQueriable::duplicate(std::unique_ptr<Queriable>& dup) const
{
