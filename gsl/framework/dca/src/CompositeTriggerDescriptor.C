...
CompositeTriggerDescriptor::CompositeTriggerDescriptor(Simulation& s)
   : TriggerType("CompositeTrigger", "CompositeTrigger", 
...
void CompositeTriggerDescriptor::getQueriable(
...
Trigger* CompositeTriggerDescriptor::getTrigger(std::vector<DataItem*> const & args)
{
   Trigger* rval = new CompositeTrigger(_sim, args);
...
Trigger* CompositeTriggerDescriptor :: getTrigger(NDPairList& ndp)
{
   Trigger* rval = 0;
   ...
   if(itr7 == end) {
	 ...
	 args.push_back((*itr1)->getDataItem());
      ...
      rval = new CompositeTrigger(_sim, args);
      _triggerList.push_back(rval);
      TriggerDataItem* tdi = new TriggerDataItem;
      tdi->setTrigger(rval);
      _instances.push_back(tdi);
...
void CompositeTriggerDescriptor::duplicate(
   std::unique_ptr<TriggerType>& dup) const
{
   dup.reset(new CompositeTriggerDescriptor(*this));
}

CompositeTriggerDescriptor:: ~CompositeTriggerDescriptor()
{
}
