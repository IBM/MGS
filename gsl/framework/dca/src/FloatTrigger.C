   ...
   : TriggerBase(), _sim(sim)
{
   ...
   if (descriptionDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to StringDataItem failed on FloatTrigger!"<<std::endl;
      exit(-1);
   }
   ...
   if (ptrSvcDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to ServiceDataItem failed on FloatTrigger!"<<std::endl;
      exit(-1);
   }
   ++iter;
   ...
   if (criterionDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to NumericDataItem failed on FloatTrigger!"<<std::endl;
      exit(-1);
   }
   ...
   if (phaseDI == 0) {
      std::cerr<<"Dynamic cast of DataItem to PhaseDataItem failed on FloatTrigger!"<<std::endl;
      exit(-1);
   }
...
{
   if (_op == ">") _evaluator = &FloatTrigger::isGreaterThan;
   else if (_op == "<") _evaluator = &FloatTrigger::isLessThan;
   else if (_op == "==") _evaluator = &FloatTrigger::isEqual;
   else if (_op == "!=") _evaluator = &FloatTrigger::isNotEqual;
   else if ((_op == ">=") || (_op == "=>")) _evaluator = &FloatTrigger::isGreaterThanOrEqual;
   else if ((_op == "<=") || (_op == "=<")) _evaluator = &FloatTrigger::isLessThanOrEqual;
   else {
...
bool FloatTrigger::isEqual()
{
   return (*_service) == _criterion;
}


bool FloatTrigger::isGreaterThan()
{
   return (*_service) > _criterion;
}


bool FloatTrigger::isLessThan()
{
   return (*_service) < _criterion;
}


bool FloatTrigger::isNotEqual()
{
   return (*_service) != _criterion;
}


bool FloatTrigger::isGreaterThanOrEqual()
{
   return (*_service) >= _criterion;
}


bool FloatTrigger::isLessThanOrEqual()
{
   return (*_service) <= _criterion;
}
