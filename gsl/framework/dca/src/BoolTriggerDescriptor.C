...
BoolTriggerDescriptor::BoolTriggerDescriptor(Simulation& s)
   : TriggerType("BoolTrigger", "BoolTrigger", "Can trigger on any bool."), 
     _sim(s)
{
...
void BoolTriggerDescriptor::getQueriable(
...
Trigger* BoolTriggerDescriptor::getTrigger(std::vector<DataItem*> const & args)
{
   Trigger* rval = new BoolTrigger(_sim, args);
...
Trigger* BoolTriggerDescriptor :: getTrigger(NDPairList& ndp)
{
   Trigger* rval = 0;
   ...
   if(itr2 == end) {
      throw SyntaxErrorException("Service not found in BoolTriggerDescriptor NDPList!");
   }
   else if(itr3 == end) {
      ...
      throw SyntaxErrorException("Delay not found in BoolTriggerDescriptor NDPList!");
   }
   else {
	 ...
	 args.push_back((*itr)->getDataItem());
      ...
      rval = new BoolTrigger(_sim, args);
      _triggerList.push_back(rval);
      TriggerDataItem* tdi = new TriggerDataItem;
      tdi->setTrigger(rval);
      _instances.push_back(tdi);
...
void BoolTriggerDescriptor::duplicate(
   std::unique_ptr<TriggerType>& dup) const
{
   dup.reset(new BoolTriggerDescriptor(*this));
}

BoolTriggerDescriptor:: ~BoolTriggerDescriptor()
{
}
