...
SimulationPublisher::SimulationPublisher(const SimulationPublisher& rv)
   : _sim(rv._sim), _name(rv._name), _description(rv._description), 
...
Service* SimulationPublisher::getService(const std::string& serviceRequested)
{
   ...
   for (std::vector<Service*>::iterator iter=_services.begin(); iter != end; ++iter) {
   ...
   if (rval == 0) {
      std::cerr<<"Requested service "<<serviceRequested<<" not found in SimulationPublisher's Service List!"<<std::endl;
      exit(-1);
   }
...
const std::vector<TriggerType*>& SimulationPublisher::getTriggerDescriptors() const
{
   return _triggerDescriptors;
}


TriggerType* SimulationPublisher::getTriggerDescriptor(const std::string& triggerDescriptorRequested)
{
   std::vector<TriggerType*>::iterator end = _triggerDescriptors.end();
   for (std::vector<TriggerType*>::iterator iter = _triggerDescriptors.begin(); iter != end; iter++) {
      if ((*iter)->getName()==triggerDescriptorRequested)
         return *iter;
   }
   std::cerr<<"Requested trigger descriptor not found in Trigger Descriptor List!"<<std::endl;
   return 0;                     //Throw exception here.
}


std::string SimulationPublisher::getName() const
{
   return _name;
}


std::string SimulationPublisher::getDescription() const
{
   return _description;
}
