...
template <typename SenderType>
SenderOutputStream<SenderType>::SenderOutputStream(int peer, int tag, Simulation* sim)
   : _current(0) {
	 
   _buffers[0] = new OutputBuffer<BUFFERSIZE>;
   _buffers[1] = new OutputBuffer<BUFFERSIZE>;
   _senders[0] = new SenderType(_buffers[0]->_data, BUFFERSIZE, peer,
				tag, sim);
   _senders[1] = new SenderType(_buffers[1]->_data, BUFFERSIZE, peer,
				tag, sim);	    
}
...
void SenderOutputStream<SenderType>::flush() {
  if (_buffers[_current]->_index != 0) {
    ...
    _senders[_current]->sendRequest(_buffers[_current]->_index); // this sender must not be active
...
void SenderOutputStream<SenderType>::write(const char* data, int size) {
  ...
  do {
	...
	flush();
      ...
      _senders[_current]->complete();  // this must be called to make the sender inactive and prepare for flush
