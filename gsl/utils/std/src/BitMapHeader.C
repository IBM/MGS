...
BitMapHeader::BitMapHeader(char* id, uint32_t fsz, uint16_t rsvd1, uint16_t rsvd2, uint32_t bmDataOffset, uint32_t bmHeaderSize, uint32_t w, uint32_t h, uint16_t p, uint16_t bpp, uint32_t c, uint32_t bmDataSize, uint32_t hRes, uint32_t vRes, uint32_t clrs, uint32_t impClrs) {
...
void BitMapHeader::writeToFile(FILE* out) {
  ...
  fileHeader.fileSize=FILEHEADER_SIZE+INFOHEADER_SIZE+PALETTE_SIZE+( (infoHeader.width + rowPadding)*infoHeader.height );
...
void BitMapHeader::readGrayscales(FILE* in, std::auto_ptr<char>& grays) 
{
  ...
  for (unsigned int y=0; y<infoHeader.height; ++y) {
    for (unsigned int x=0; x<infoHeader.width; ++x) {
      if (grayscale) s=fread(&g[INDEX(x,y)], sizeof(char), 1, in);
      else {
	s=fread(&c, sizeof(char), 1, in);
	g[INDEX(x,y)] = char(0.3*float(palette[c*4]) + 0.59*float(palette[c*4+1]) + 0.11*float(palette[c*4+2]));
      }
    }
    for (unsigned int i=0; i<rowPadding; ++i) {
      s=fread(&c, sizeof(char), 1, in);
...
void BitMapHeader::readColors(FILE* in, std::auto_ptr<char>& reds, std::auto_ptr<char>& greens, std::auto_ptr<char>& blues) 
{
  size_t s=fread(fileHeader.identifier, sizeof(char), 2, in);
  if(feof(in)) {
    std::cerr<<"Unexpected EOF in BitMapHeader!"<<std::endl;
...
void BitMapHeader::readRedGreenOpponents(FILE* in, std::auto_ptr<char>& redGreenOpponents) 
{
  size_t s=fread(fileHeader.identifier, sizeof(char), 2, in);
  if(feof(in)) {
    std::cerr<<"Unexpected EOF in BitMapHeader!"<<std::endl;
...
void BitMapHeader::readYellowBlueOpponents(FILE* in, std::auto_ptr<char>& yellowBlueOpponents) 
{
  size_t s=fread(fileHeader.identifier, sizeof(char), 2, in);
  if(feof(in)) {
    std::cerr<<"Unexpected EOF in BitMapHeader!"<<std::endl;
...
void BitMapHeader::hsv2rgb(float h, float s, float v, float& r,
