...
void ReversalPotentialDisplay::initialize(RNG& rng) 
{
  if (E_channel.size()>0 || E_synapse.size()>0) {
    ...
    if (E_channel.size()>0) {
      if (indices.size()==0) {
	for (int i=0; i<E_channel.size(); ++i) {
	  if (E_channel[i]->size()==0)
	    std::cerr<<"WARNING : No channel branch information available. Consider initializing ReversalPotentialDisplay at a later phase."<<std::endl;
	  for (int j=0; j<E_channel[i]->size(); ++j) {
    ...
    if (E_synapse.size()>0) {
      for (int i=0; i<E_synapse.size(); ++i) {
...
void ReversalPotentialDisplay::dataCollection(Trigger* trigger, NDPairList* ndPairList) 
{
  if (E_channel.size()>0 || E_synapse.size()>0) {
    (*outFile)<<float(getSimulation().getIteration())**deltaT;
    if (E_channel.size()>0) {
      if (indices.size()==0) {
	ShallowArray< ShallowArray <float>* >::iterator it1 = E_channel.begin(),
	  end1 = E_channel.end();
	for (; it1!=end1; ++it1) {
    ...
    if (E_synapse.size()>0) {
      ShallowArray<float*>::iterator it = E_synapse.begin(),
	end = E_synapse.end();
      for (; it!=end; ++it) {
...
void ReversalPotentialDisplay::setUpPointers(const String& CG_direction, const String& CG_component, NodeDescriptor* CG_node, Edge* CG_edge, VariableDescriptor* CG_variable, Constant* CG_constant, CG_ReversalPotentialDisplayInAttrPSet* CG_inAttrPset, CG_ReversalPotentialDisplayOutAttrPSet* CG_outAttrPset) 
{
...
void ReversalPotentialDisplay::duplicate(std::auto_ptr<ReversalPotentialDisplay>& dup) const
{
...
void ReversalPotentialDisplay::duplicate(std::auto_ptr<CG_ReversalPotentialDisplay>& dup) const
{
   dup.reset(new ReversalPotentialDisplay(*this));
}
