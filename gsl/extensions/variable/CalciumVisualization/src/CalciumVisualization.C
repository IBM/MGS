...
CalciumVisualization::CalciumVisualization()
  : CG_CalciumVisualization(), _outFile(0), _rank(-1), _size(-1), _isIoNode(false),
    _calciumBuffs(0), _nBuffs(0), _nSends(0), _nKreceives(0), _nVreceives(0)
{
} 
...
void CalciumVisualization::initialize(RNG& rng) 
{
  ...
  assert(deltaT);
  std::map<CompartmentKey, float*, CompartmentKey::compare> calciumMap;
  if (Ca.size()>0) {
  ...
  if (_nBuffs>0) {
    ...
    for (int i=0; i<localDataSize; ++i, ++calciumMapIter) {
      if (i%BUFF_SIZE!=0 && calciumMapIter->second==_marshallPatterns.back().first+1)
	_marshallPatterns.back().second++;
      else
	_marshallPatterns.push_back(std::pair<float*, int>(calciumMapIter->second, 1));
    }
    if (!_isIoNode) {
...
void CalciumVisualization::dataCollection(Trigger* trigger, NDPairList* ndPairList) 
{
  if (_nBuffs>0) {
    int destIO=_ioNodes[_rank % N_IO_NODES];
    if (!_isIoNode) {
      ...
      for (int i=0; i<_nBuffs; ++i) 
	requests[i].second=_calciumBuffs[i];
...
void CalciumVisualization::setUpPointers(const String& CG_direction, const String& CG_component, NodeDescriptor* CG_node, Edge* CG_edge, VariableDescriptor* CG_variable, Constant* CG_constant, CG_CalciumVisualizationInAttrPSet* CG_inAttrPset, CG_CalciumVisualizationOutAttrPSet* CG_outAttrPset) 
{
...
void CalciumVisualization::duplicate(std::auto_ptr<CalciumVisualization>& dup) const
{
...
void CalciumVisualization::duplicate(std::auto_ptr<CG_CalciumVisualization>& dup) const
{
   dup.reset(new CalciumVisualization(*this));
}
