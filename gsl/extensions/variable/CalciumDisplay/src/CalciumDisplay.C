...
void CalciumDisplay::dataCollection(Trigger* trigger, NDPairList* ndPairList) 
{
  if (Ca.size()>0) {
    ...
    if (indices.size()==0) {
      ShallowArray< ShallowArray <float>* >::iterator it1 = Ca.begin(),
	end1 = Ca.end();
      for (; it1!=end1; ++it1) {
	...
	for (; it2!=end2; ++it2)
      ...
      ShallowArray< ShallowArray <float>* >::iterator it1=Ca.begin(), end1=Ca.end();
...
void CalciumDisplay::setUpPointers(const String& CG_direction, const String& CG_component, NodeDescriptor* CG_node, Edge* CG_edge, VariableDescriptor* CG_variable, Constant* CG_constant, CG_CalciumDisplayInAttrPSet* CG_inAttrPset, CG_CalciumDisplayOutAttrPSet* CG_outAttrPset) 
{
...
CalciumDisplay::CalciumDisplay() 
  : CG_CalciumDisplay(), outFile(0)
{
}
