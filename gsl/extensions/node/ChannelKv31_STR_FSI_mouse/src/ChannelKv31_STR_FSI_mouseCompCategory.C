...
#include "Mgs.h"
#include "ChannelKv31_STR_FSI_mouseCompCategory.h"
...
ChannelKv31_STR_FSI_mouseCompCategory::ChannelKv31_STR_FSI_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKv31_STR_FSI_mouseCompCategory(sim, modelName, ndpList)
{
}

void ChannelKv31_STR_FSI_mouseCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelKv31_STR_FSI_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total Kv31 Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
