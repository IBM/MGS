...
#include "Lens.h"
#include "ChannelKv1_STR_FSI_mouseCompCategory.h"
...
ChannelKv1_STR_FSI_mouseCompCategory::ChannelKv1_STR_FSI_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKv1_STR_FSI_mouseCompCategory(sim, modelName, ndpList)
{
 // step 1.
...
void ChannelKv1_STR_FSI_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total Kv1 Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}



void ChannelKv1_STR_FSI_mouseCompCategory::computeE(RNG& rng) 
{
}
