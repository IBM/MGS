...
#include "Lens.h"
#include "ChannelKDR_STR_MSN_mouseCompCategory.h"
...
ChannelKDR_STR_MSN_mouseCompCategory::ChannelKDR_STR_MSN_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKDR_STR_MSN_mouseCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelKDR_STR_MSN_mouseCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelKDR_STR_MSN_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total KDR STR MSN mouse Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
