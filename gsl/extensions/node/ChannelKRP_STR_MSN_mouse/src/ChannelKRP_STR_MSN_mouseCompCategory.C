...
#include "Lens.h"
#include "ChannelKRP_STR_MSN_mouseCompCategory.h"
...
ChannelKRP_STR_MSN_mouseCompCategory::ChannelKRP_STR_MSN_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKRP_STR_MSN_mouseCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelKRP_STR_MSN_mouseCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelKRP_STR_MSN_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total KRP STR MSN mouse Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
