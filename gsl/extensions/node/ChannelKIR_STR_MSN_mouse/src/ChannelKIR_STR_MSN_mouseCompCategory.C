...
#include "Mgs.h"
#include "ChannelKIR_STR_MSN_mouseCompCategory.h"
...
ChannelKIR_STR_MSN_mouseCompCategory::ChannelKIR_STR_MSN_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKIR_STR_MSN_mouseCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelKIR_STR_MSN_mouseCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelKIR_STR_MSN_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total KIR STR MSN mouse Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
