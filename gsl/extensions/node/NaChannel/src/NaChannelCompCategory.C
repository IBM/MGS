...
#include "Lens.h"
#include "NaChannelCompCategory.h"
#include "NDPairList.h"
#include "CG_NaChannelCompCategory.h"
#include <math.h>
//#define DEBUG_HH

NaChannelCompCategory::NaChannelCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_NaChannelCompCategory(sim, modelName, ndpList)
{
}

void NaChannelCompCategory::computeE_Na(RNG& rng) 
{
...
void NaChannelCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total Na Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
