...
#include "Mgs.h"
#include "ChannelLeak_STR_MSN_mouse.h"
...
void ChannelLeak_STR_MSN_mouse::update(RNG& rng) 
{
 ...
 for (unsigned i=0; i<branchData->size; ++i) 
  {
    dyn_var_t v=(*V)[i];
    g[i] = gbar[i];
    Iion[i] = g[i] * (v-getSharedMembers().E_leak[0]);

}
}

void ChannelLeak_STR_MSN_mouse::initialize(RNG& rng) 
{
  assert(branchData);
...
ChannelLeak_STR_MSN_mouse::~ChannelLeak_STR_MSN_mouse() 
{
}
