#include "Lens.h"
#include "ChannelKIRCompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelKIRCompCategory.h"

#include "NumberUtils.h"

ChannelKIRCompCategory::ChannelKIRCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKIRCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelKIRCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelKIRCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total KIR Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
