...
#include "Mgs.h"
#include "VoltageAdapterCompCategory.h"
#include "NDPairList.h"
#include "CG_VoltageAdapterCompCategory.h"

VoltageAdapterCompCategory::VoltageAdapterCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_VoltageAdapterCompCategory(sim, modelName, ndpList)
{
}
...
void VoltageAdapterCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total VoltageAdapter = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean,
           std);
}
