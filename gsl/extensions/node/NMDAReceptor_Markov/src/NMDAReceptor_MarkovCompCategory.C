#include "Lens.h"
#include "NMDAReceptor_MarkovCompCategory.h"
...
NMDAReceptor_MarkovCompCategory::NMDAReceptor_MarkovCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_NMDAReceptor_MarkovCompCategory(sim, modelName, ndpList)
{
}

// GOAL:
//  1. find Q10 adjustment
void NMDAReceptor_MarkovCompCategory::computeTadj(RNG& rng) 
{
...
void NMDAReceptor_MarkovCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total NMDA(Markov) synapse = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
