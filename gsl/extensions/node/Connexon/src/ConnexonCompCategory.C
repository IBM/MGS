...
#include "Lens.h"
#include "ConnexonCompCategory.h"
#include "NDPairList.h"
#include "CG_ConnexonCompCategory.h"
#include <math.h>

ConnexonCompCategory::ConnexonCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ConnexonCompCategory(sim, modelName, ndpList)
{
}

void ConnexonCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total Connexon = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
