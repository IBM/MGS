...
#include "Lens.h"
#include "Connexon_VmCompCategory.h"
#include "NDPairList.h"
#include "CG_Connexon_VmCompCategory.h"
#include <math.h>

Connexon_VmCompCategory::Connexon_VmCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_Connexon_VmCompCategory(sim, modelName, ndpList)
{
}

void Connexon_VmCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total Connexon_Vm = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
