...
#include "Lens.h"
#include "ChannelKDR_GPe_mouseCompCategory.h"
...
ChannelKDR_GPe_mouseCompCategory::ChannelKDR_GPe_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKDR_GPe_mouseCompCategory(sim, modelName, ndpList)
{
}

void ChannelKDR_GPe_mouseCompCategory::computeE(RNG& rng) 
{  // step 1.
...
void ChannelKDR_GPe_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total KDR Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
