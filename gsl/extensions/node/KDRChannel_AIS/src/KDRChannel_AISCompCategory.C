...
#include "Lens.h"
#include "KDRChannel_AISCompCategory.h"
#include "NDPairList.h"
#include "CG_KDRChannel_AISCompCategory.h"
#include <math.h>
//#define DEBUG_HH

KDRChannel_AISCompCategory::KDRChannel_AISCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_KDRChannel_AISCompCategory(sim, modelName, ndpList)
{
}

void KDRChannel_AISCompCategory::computeE_KDR(RNG& rng) 
{
...
void KDRChannel_AISCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total KDR Channel_AIS = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
