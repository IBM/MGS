#include "Lens.h"
#include "ChannelCaR_GHKCompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelCaR_GHKCompCategory.h"

#include "NumberUtils.h" //new

ChannelCaR_GHKCompCategory::ChannelCaR_GHKCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelCaR_GHKCompCategory(sim, modelName, ndpList)
{
}

// GOAL:
//  1. find Q10 adjustment
void ChannelCaR_GHKCompCategory::computeTadj(RNG& rng)
{
...
void ChannelCaR_GHKCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total CaR_GHK Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
