...
#include "Mgs.h"
#include "ChannelKCNK_GHKCompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelKCNK_GHKCompCategory.h"
#include "NumberUtils.h"
#include <mpi.h>

ChannelKCNK_GHKCompCategory::ChannelKCNK_GHKCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKCNK_GHKCompCategory(sim, modelName, ndpList)
{
}

// GOAL:
//  1. compute Erev using Nernst equation
//  2. find Q10 adjustment
void ChannelKCNK_GHKCompCategory::computeTadj(RNG& rng) 
{
  // step 1.
...
void ChannelKCNK_GHKCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total KCNK Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
