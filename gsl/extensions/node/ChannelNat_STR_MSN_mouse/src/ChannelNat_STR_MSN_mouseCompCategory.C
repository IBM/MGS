...
#include "Lens.h"
#include "ChannelNat_STR_MSN_mouseCompCategory.h"
...
ChannelNat_STR_MSN_mouseCompCategory::ChannelNat_STR_MSN_mouseCompCategory(Simulation& sim, 
                                                                           const std::string& modelName, 
                                                                           const NDPairList& ndpList) 
   : CG_ChannelNat_STR_MSN_mouseCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelNat_STR_MSN_mouseCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelNat_STR_MSN_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total Nat STR MSN mouse Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
