...
#include "Mgs.h"
#include "ChannelNasCompCategory.h"
...
ChannelNasCompCategory::ChannelNasCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelNasCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelNasCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelNasCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total Nas Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
