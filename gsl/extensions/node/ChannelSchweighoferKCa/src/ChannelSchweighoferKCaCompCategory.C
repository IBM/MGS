...
#include "Lens.h"
#include "ChannelSchweighoferKCaCompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelSchweighoferKCaCompCategory.h"
#include <math.h>
//#define DEBUG_HH

ChannelSchweighoferKCaCompCategory::ChannelSchweighoferKCaCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelSchweighoferKCaCompCategory(sim, modelName, ndpList)
{
}

void ChannelSchweighoferKCaCompCategory::computeE(RNG& rng) 
{
...
void ChannelSchweighoferKCaCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total KCa Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
