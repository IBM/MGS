#include "Lens.h"
#include "ChannelCaN_GHKCompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelCaN_GHKCompCategory.h"

#include "NumberUtils.h" //new

ChannelCaN_GHKCompCategory::ChannelCaN_GHKCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelCaN_GHKCompCategory(sim, modelName, ndpList)
{
}

// GOAL:
//  1. find Q10 adjustment
void ChannelCaN_GHKCompCategory::computeTadj(RNG& rng)
{
...
void ChannelCaN_GHKCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total CaN_GHK Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
