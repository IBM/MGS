...
void ChannelCaN_GHK::update(RNG& rng) 
{
  ...
  {
    dyn_var_t v = (*V)[i];
#if CHANNEL_CaN == CaN_WOLF_2005
    // NOTE: Some models use m_inf and tau_m to estimate m
    dyn_var_t ma = 0.1157 * (v + 17.19) / (exp((v + 17.19) / 15.22) - 1);
    dyn_var_t mb = 1.15 * exp((v) / 23.82);
    dyn_var_t tau_m = 1.0 / (ma + mb);
    dyn_var_t qm = dt * getSharedMembers().Tadj / (tau_m * 2);

    dyn_var_t tau_h = 23.33;  //[msec]
...
void ChannelCaN_GHK::setPointers(const String& CG_direction, const String& CG_component, NodeDescriptor* CG_node, Edge* CG_edge, VariableDescriptor* CG_variable, Constant* CG_constant, CG_ChannelCaN_GHKInAttrPSet* CG_inAttrPset, CG_ChannelCaN_GHKOutAttrPSet* CG_outAttrPset) 
{
  _cptindex = CG_inAttrPset->idx;
}

void ChannelCaN_GHK::initialize_others()
{
}

ChannelCaN_GHK::~ChannelCaN_GHK() 
{
}
