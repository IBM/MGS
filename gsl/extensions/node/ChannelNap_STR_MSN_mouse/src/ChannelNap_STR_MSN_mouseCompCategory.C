...
#include "Mgs.h"
#include "ChannelNap_STR_MSN_mouseCompCategory.h"
...
ChannelNap_STR_MSN_mouseCompCategory::ChannelNap_STR_MSN_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelNap_STR_MSN_mouseCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelNap_STR_MSN_mouseCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelNap_STR_MSN_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total Nap STR MSN moues Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
