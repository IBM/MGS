...
#include "Lens.h"
#include "NazeSORNInhUnitCompCategory.h"
...
NazeSORNInhUnitCompCategory::NazeSORNInhUnitCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_NazeSORNInhUnitCompCategory(sim, modelName, ndpList)
{
}


void NazeSORNInhUnitCompCategory::initializeShared(RNG& rng) 
{
...
void NazeSORNInhUnitCompCategory::outputWeightsShared(RNG& rng) 
{
  if (SHD.collectWeightsOn.size()>0) {
	...
	if (n==rank) {
	  ...
	  ShallowArray<NazeSORNInhUnit>::iterator it = _nodes.begin();
	  ShallowArray<NazeSORNInhUnit>::iterator end = _nodes.end();
...
void NazeSORNInhUnitCompCategory::inputWeightsShared(RNG& rng) 
{
  if (SHD.loadWeightsOn.size()>0) {
	...
	if (n==rank) {
	  ...
	  if (fsE2I.good()) {
	    ...
	    ShallowArray<NazeSORNInhUnit>::iterator it;//, end;
	    while (!fsE2I.eof()) {
	  ...
	  if (SHD.inFiles.size()>1) {
	    ...
	    if(fsTIs.good()){
	      int idx; float val;
	      ShallowArray<NazeSORNInhUnit>::iterator it;
	      while (!fsTIs.eof()) {
		  fsTIs>>idx>>val;
		  ShallowArray<NazeSORNInhUnit>::iterator end = _nodes.end();
		  for (it = _nodes.begin(); it != end; ++it) {
