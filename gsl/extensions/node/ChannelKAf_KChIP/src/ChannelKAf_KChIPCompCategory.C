#include "Lens.h"
#include "ChannelKAf_KChIPCompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelKAf_KChIPCompCategory.h"
#include <mpi.h>
#include "NumberUtils.h"

ChannelKAf_KChIPCompCategory::ChannelKAf_KChIPCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelKAf_KChIPCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelKAf_KChIPCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelKAf_KChIPCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total KAf(KChIP) Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
