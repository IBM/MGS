...
#include "Lens.h"
#include "NaChannel_AISCompCategory.h"
#include "NDPairList.h"
#include "CG_NaChannel_AISCompCategory.h"
#include <math.h>
//#define DEBUG_HH

NaChannel_AISCompCategory::NaChannel_AISCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_NaChannel_AISCompCategory(sim, modelName, ndpList)
{
}

void NaChannel_AISCompCategory::computeE_Na(RNG& rng) 
{
...
void NaChannel_AISCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total Na Channel_AIS = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
