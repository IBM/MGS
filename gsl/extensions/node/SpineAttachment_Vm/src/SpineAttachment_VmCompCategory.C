...
SpineAttachment_VmCompCategory::SpineAttachment_VmCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
...
void SpineAttachment_VmCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total SpineAttachment_Vm = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
