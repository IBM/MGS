...
AMPAReceptorCompCategory::AMPAReceptorCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
...
void AMPAReceptorCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total AMPA synapse = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
