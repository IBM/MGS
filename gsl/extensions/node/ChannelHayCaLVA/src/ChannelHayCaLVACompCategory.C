...
#include "Lens.h"
#include "ChannelHayCaLVACompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelHayCaLVACompCategory.h"
#include <math.h>
//#define DEBUG_HH

ChannelHayCaLVACompCategory::ChannelHayCaLVACompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelHayCaLVACompCategory(sim, modelName, ndpList)
{
}

void ChannelHayCaLVACompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total Ca_LVA Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
