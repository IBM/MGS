...
void IP3Concentration::initializeCompartmentData(RNG& rng)
{
  ...
  assert(IP3_new.size() == size);
  assert(distalDimensions.size() == distalInputs.size());

  // allocate data
  if (IP3_cur.size() != size) IP3_cur.increaseSizeTo(size);
...
//|dyn_var_t IP3Concentration::getLambda(DimensionStruct* a, DimensionStruct* b)
//|{
...
//|    distance = fabs(b->dist2soma - a->dist2soma);
//|  }
//|  //return (M_PI * Rb * Rb * getSharedMembers().D_IP3 /
