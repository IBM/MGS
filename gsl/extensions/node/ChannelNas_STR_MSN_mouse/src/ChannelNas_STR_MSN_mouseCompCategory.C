...
#include "Lens.h"
#include "ChannelNas_STR_MSN_mouseCompCategory.h"
...
ChannelNas_STR_MSN_mouseCompCategory::ChannelNas_STR_MSN_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelNas_STR_MSN_mouseCompCategory(sim, modelName, ndpList)
{
}
...
void ChannelNas_STR_MSN_mouseCompCategory::computeE(RNG& rng) 
{
  // step 1.
...
void ChannelNas_STR_MSN_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total Nas STR MSN mouse Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
