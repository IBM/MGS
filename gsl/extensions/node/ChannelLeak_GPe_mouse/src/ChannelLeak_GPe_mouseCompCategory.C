...
#include "Lens.h"
#include "ChannelLeak_GPe_mouseCompCategory.h"
...
ChannelLeak_GPe_mouseCompCategory::ChannelLeak_GPe_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelLeak_GPe_mouseCompCategory(sim, modelName, ndpList)
{
}
//
...
void ChannelLeak_GPe_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total GPe Leak Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
