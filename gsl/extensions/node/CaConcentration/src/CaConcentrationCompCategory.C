...
CaConcentrationCompCategory::CaConcentrationCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
...
#ifdef DEBUG_HH
    std::cerr<<getSimulation().getRank()<<" : CaConcentrationes : "<<_nodes.size()<<" [ ";
...
void CaConcentrationCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total CaConcentration = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);

  ////

  totalCount=localCount=0;
  ShallowArray<CaConcentration>::iterator nodesIter=_nodes.begin(), nodesEnd=_nodes.end();
  ...
  if (getSimulation().getRank()==0) printf("Total Ca Compartment = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
