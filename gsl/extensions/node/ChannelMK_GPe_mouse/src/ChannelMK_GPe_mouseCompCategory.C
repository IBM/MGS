...
#include "Lens.h"
#include "ChannelMK_GPe_mouseCompCategory.h"
...
ChannelMK_GPe_mouseCompCategory::ChannelMK_GPe_mouseCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelMK_GPe_mouseCompCategory(sim, modelName, ndpList)
{
}

void ChannelMK_GPe_mouseCompCategory::computeE(RNG& rng) 
{
// step 1.
...
void ChannelMK_GPe_mouseCompCategory::count()
{
  ...
  if (getSimulation().getRank() == 0)
    printf("Total MK Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount,
           mean, std);
}
