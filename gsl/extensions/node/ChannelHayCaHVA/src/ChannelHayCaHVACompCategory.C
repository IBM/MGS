...
#include "Lens.h"
#include "ChannelHayCaHVACompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelHayCaHVACompCategory.h"
#include <math.h>
//#define DEBUG_HH

ChannelHayCaHVACompCategory::ChannelHayCaHVACompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelHayCaHVACompCategory(sim, modelName, ndpList)
{
}

void ChannelHayCaHVACompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total Ca_HVA Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
