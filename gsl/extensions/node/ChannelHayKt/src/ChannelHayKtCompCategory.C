...
#include "Lens.h"
#include "ChannelHayKtCompCategory.h"
#include "NDPairList.h"
#include "CG_ChannelHayKtCompCategory.h"
#include <math.h>
//#define DEBUG_HH

ChannelHayKtCompCategory::ChannelHayKtCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_ChannelHayKtCompCategory(sim, modelName, ndpList)
{
}

void ChannelHayKtCompCategory::computeE(RNG& rng) 
{
...
void ChannelHayKtCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total Kt Channel = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
