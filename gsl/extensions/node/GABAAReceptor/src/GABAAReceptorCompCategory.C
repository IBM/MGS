...
#include "Lens.h"
#include "GABAAReceptorCompCategory.h"
#include "NDPairList.h"
#include "CG_GABAAReceptorCompCategory.h"
#include <math.h>

GABAAReceptorCompCategory::GABAAReceptorCompCategory(Simulation& sim, const std::string& modelName, const NDPairList& ndpList) 
   : CG_GABAAReceptorCompCategory(sim, modelName, ndpList)
{
}

void GABAAReceptorCompCategory::count() 
{
  ...
  if (getSimulation().getRank()==0) printf("Total GABAA synapse = %lld, Mean = %lf, StDev = %lf\n", totalCount, mean, std);
}
