...
#include "SrcRefSumRsqrdInvWeightModifier.h"
...
void SrcRefSumRsqrdInvWeightModifier::userInitialize(LensContext* CG_c, Functor*& f, int& maxDim, bool& setDistance) 
{
  assert(maxDim>0);
...
std::auto_ptr<ParameterSet> SrcRefSumRsqrdInvWeightModifier::userExecute(LensContext* CG_c) 
{
   ...
   if (psdi==0) {
	 ...
	 "SrcRefSumRsqrdInvWeightModifier: functor did not return a Parameter Set");
   }
...
SrcRefSumRsqrdInvWeightModifier::SrcRefSumRsqrdInvWeightModifier() 
   : CG_SrcRefSumRsqrdInvWeightModifierBase()
{
}
...
void SrcRefSumRsqrdInvWeightModifier::duplicate(std::auto_ptr<SrcRefSumRsqrdInvWeightModifier>& dup) const
{
   dup.reset(new SrcRefSumRsqrdInvWeightModifier(*this));
}

void SrcRefSumRsqrdInvWeightModifier::duplicate(std::auto_ptr<Functor>& dup) const
{
   dup.reset(new SrcRefSumRsqrdInvWeightModifier(*this));
}

void SrcRefSumRsqrdInvWeightModifier::duplicate(std::auto_ptr<CG_SrcRefSumRsqrdInvWeightModifierBase>& dup) const
{
   dup.reset(new SrcRefSumRsqrdInvWeightModifier(*this));
}
