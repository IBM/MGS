...
void ServiceConnectorFunctor::userExecute(LensContext* CG_c, std::vector<DataItem*>::const_iterator begin, std::vector<DataItem*>::const_iterator end) 
{
#ifdef HAVE_MPI
//   int mySpaceId;
//   MPI_Comm_rank(MPI_COMM_WORLD, &mySpaceId);
   ...
   int size = end - begin;
   if ((size < 4) || (size % 2)) { // (size % 2) == -size is odd-
...
ServiceConnectorFunctor::ServiceConnectorFunctor() 
   : CG_ServiceConnectorFunctorBase(), _sourceConstantDI(0),
     _sourceVariableDI(0), _destinationVariableDI(0),
     _sourceEdgeSetDI(0), _destinationEdgeSetDI(0),
     _sourceNodeSetDI(0), _destinationNodeSetDI(0), _execContext(0)
{
}
...
void ServiceConnectorFunctor::duplicate(std::auto_ptr<ServiceConnectorFunctor>& dup) const
{
...
void ServiceConnectorFunctor::duplicate(std::auto_ptr<CG_ServiceConnectorFunctorBase>& dup) const
{
   dup.reset(new ServiceConnectorFunctor(*this));
}
...
//   MPI_Comm_rank(MPI_COMM_WORLD, &mySpaceId);
   ...
   if (_destinationType == _Variable) {
      // need to determine both ends' partitionID and decide which whether need to create
      // an proxy. It is always that create an proxy for the pre-node or pre-variable.
      // similiar to the case in LensConnector: if both ends are not in current memory
      // space we do nothing. 
      Variable* av = _destinationVariableDI->getVariable()->getVariable();
      if (av) {
         connectToService(
//	    service, _destinationVariableDI->getVariable(), acceptorName);
            service, _destinationVariableDI->getVariable()->getVariable(), acceptorName);   // added by Jizhu Lu on 03/16/2006
      }
   } else if (_destinationType == _NodeSet) {
      connectToService(
	 service, _destinationNodeSetDI->getNodeSet(), acceptorName);
   } else if (_destinationType == _EdgeSet) {
      connectToService(
	 service, _destinationEdgeSetDI->getEdgeSet(), acceptorName);
   } else {
      assert(0);
   }
}
...
//      if ((*it)->getNode() && (*it)->getNode()->hasService())     // modified by Jizhu Lu on 01/13/2006   // commented out by Jizhu Lu on 06/14/2006
...
ServiceConnectorElement::ServiceConnectorElement(
...
ServiceConnectorElement::ServiceConnectorElement(
   ...
   : _serviceName(rv._serviceName), _acceptorName(rv._acceptorName), 
     _functor(0)
{
   copyOwnedHeap(rv);
}
...
ServiceConnectorFunctor::ServiceConnectorElement::operator=(
   const ServiceConnectorFunctor::ServiceConnectorElement& rv)
{
   if (this != &rv) {
      destructOwnedHeap();
...
void ServiceConnectorFunctor::
ServiceConnectorElement::establishConnection(ServiceConnectorFunctor& parent)
{
   if (_serviceName == "") {
...
void ServiceConnectorFunctor::
ServiceConnectorElement::stringConnection(ServiceConnectorFunctor& parent)
{
#ifdef HAVE_MPI
   int mySpaceId;
   MPI_Comm_rank(MPI_COMM_WORLD, &mySpaceId);
   ...
   if (parent._sourceType == _Constant) {
      service = parent._sourceConstantDI->getConstant()->
	 getPublisher()->getService(_serviceName);
      ...
      if (!av) {
#ifdef HAVE_MPI
         int fromPartitionId = parent._execContext->sim->getGranule(variableIndex)->getPartitionId();
      ...
      for (; it != end; ++it) {
	 // @TODO Distributed local filter
//         if ((*it)->getNode() && (*it)->getNode()->hasService()) {      // modified by Jizhu Lu on 01/12/2006  // commented out by Jizhu Lu on 06/14/2006
         if ((*it)->getNode()) {      // modified by Jizhu Lu on 06/14/2006
...
void ServiceConnectorFunctor::
ServiceConnectorElement::functorConnection(ServiceConnectorFunctor& parent)
{
...
//         if ((*it)->getNode() && (*it)->getNode()->hasService()) {  // modified by Jizhu Lu on 01/13/2006   // commented out by Jizhu Lu on 06/14/2006
         if ((*it)->getNode()) {   // modified by Jizhu Lu on 06/14/2006
//            std::cerr << "ServiceConnectorElement::functorConnection(),in loop, value=" << dynamic_cast<CG_LifeNode>((*it)->getNode())->CG_get_ValueProducer_value() << std::endl;
      	    ...
      	    if (serviceDI == 0) {
      	       throw SyntaxErrorException(
      	       "Functor did not return a Service from a node in ServiceConnector.\n");
      	    }
	 ...
	 _functor->execute(parent._execContext, args, retValue);
	 serviceDI = dynamic_cast<ServiceDataItem*>(
	    retValue.get());
	 if (serviceDI == 0) {
	    throw SyntaxErrorException(
	       "Functor did not return a Service from an edge in ServiceConnector.\n");
	 }
	 parent.connectToService(serviceDI->getService(), _acceptorName);
      }
   } else {
...
void ServiceConnectorFunctor::
ServiceConnectorElement::copyOwnedHeap(
   const ServiceConnectorFunctor::ServiceConnectorElement& rv)
{
   if (rv._functor) {
...
void ServiceConnectorFunctor::
ServiceConnectorElement::destructOwnedHeap()
{
   delete _functor;
}
