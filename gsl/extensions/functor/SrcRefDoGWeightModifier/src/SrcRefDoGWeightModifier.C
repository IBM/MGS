...
std::unique_ptr<ParameterSet> SrcRefDoGWeightModifier::userExecute(LensContext* CG_c) 
{
   ...
   if (psdi==0) {
	 ...
	 "SrcRefDoGWeightModifier: functor did not return a Parameter Set");
   }
   ...
   */
   ...
   float weight = _max1*exp(-(distance*distance)/(2.0*_sigma1*_sigma1))-_max2*exp(-(distance*distance)/(2.0*_sigma2*_sigma2));
...
SrcRefDoGWeightModifier::SrcRefDoGWeightModifier() 
   : CG_SrcRefDoGWeightModifierBase()
{
}
