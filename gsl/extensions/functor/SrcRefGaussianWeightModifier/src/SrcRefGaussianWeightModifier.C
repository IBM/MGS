...
void SrcRefGaussianWeightModifier::userInitialize(GslContext* CG_c, Functor*& f, float& sigma, float& max, int& wrapDistance) 
{
  _sigma=sigma;
  _max=max;
...
std::unique_ptr<ParameterSet> SrcRefGaussianWeightModifier::userExecute(GslContext* CG_c) 
{
