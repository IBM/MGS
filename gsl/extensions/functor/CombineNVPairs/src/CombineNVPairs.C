...
std::auto_ptr<NDPairList> CombineNVPairs::userExecute(LensContext* CG_c) 
{
  DuplicatePointerArray< Functor >::iterator iter = init.fl.begin(), end = init.fl.end();
  std::auto_ptr<NDPairList> rval;
...
void CombineNVPairs::duplicate(std::auto_ptr<CombineNVPairs>& dup) const
{
   dup.reset(new CombineNVPairs(*this));
}

void CombineNVPairs::duplicate(std::auto_ptr<Functor>& dup) const
{
   dup.reset(new CombineNVPairs(*this));
}

void CombineNVPairs::duplicate(std::auto_ptr<CG_CombineNVPairsBase>& dup) const
{
   dup.reset(new CombineNVPairs(*this));
}
